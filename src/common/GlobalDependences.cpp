//===- GlobalDependencies.cpp - resolve GlobalValue dependencies ----------===//
//
...
//void GlobalDependences::getAllDependencesForValue(llvm::GlobalValue * value,
...
public:
  ...
  static void
  initializeWithGlobalDependences(GlobalDependences *_globalDependences) {
    assert(globalDependences == NULL);
    globalDependences = _globalDependences;
  }
  static void
  finalizeWithGlobalDependences(GlobalDependences *_globalDependences) {
  ...
  int getNumOfReferences() { return numOfReferences; }
  void setGlobalValue(llvm::GlobalValue *_value) { value = _value; }
  llvm::GlobalValue *getGlobalValue() { return value; }

  template <typename UserType> void processUser(UserType *user);
  ...
  void recursiveCollectAllDependences(
    ...
    if (!collected) {
      ...
      for (deps_iterator iter = dependences.begin(),
                         iter_end = dependences.end();
           iter != iter_end; iter++)
...
public:
  void getAllDependencesForValue(llvm::GlobalValue *value,
    ...
    for (std::list<GlobalValueInfo *>::iterator
             iter = dependencesList.begin(),
             iter_end = dependencesList.end();
         iter != iter_end; iter++) {
  ...
  void dropAllReferences() {
    for (FunctionsDeps::iterator iter = functionsDeps.begin(),
                                 iter_end = functionsDeps.end();
         iter != iter_end; iter++)
      iter->second.dropAllReferences();

    for (VariablesDeps::iterator iter = variablesDeps.begin(),
                                 iter_end = variablesDeps.end();
         iter != iter_end; iter++)
      iter->second.dropAllReferences();

    for (AliasesDeps::iterator iter = aliasesDeps.begin(),
                               iter_end = aliasesDeps.end();
         iter != iter_end; iter++)
      iter->second.dropAllReferences();
  }
  GlobalDependences() {
...
template <typename UserType> void GlobalValueInfo::processUser(UserType *user) {
  ...
  for (int i = 0; i < numberOfOperands; i++) {
    ...
    if (isa<GlobalValue>(*operandValue))
      dependences.push_back(globalDependences->getInfoForGlobalValue(
          cast<GlobalValue>(operandValue)));
      ...
      while (!stack.empty()) {
        ...
        for (int operandIndex = 0; operandIndex < numOfCurrentOperands;
          ...
          if (isa<GlobalValue>(*operandValue)) {
            ...
            if (std::find(stack.begin(), stack.end(), operandValue) ==
                    stack.end() &&
...
void getAllDependencesForValue(llvm::GlobalValue *value,
                               DepsByType &dependencesByType) {
  //прописать сюда периодическую очистку
  //зависимостей
...
void linkFunctionWithAllDependendes(Function *srcFunction,
  ...
  for (variable_iter iter = dependences.variables.begin(),
    ...
    if (!GV)
      GV = new GlobalVariable(
          *dstModule, I->getType()->getElementType(), I->isConstant(),
          I->getLinkage(), (Constant *)0, I->getName(), (GlobalVariable *)0,
