...
public:
  ...
  Verbose(int mode = 0)
      : mode(mode), filter(1), always(0),
        llvm::raw_fd_ostream(fileno(stdout), false, true) {}
  ...
  struct Mode {
    ...
    int operator&(Mode &other) { return this->value & other.value; }

    bool operator==(Mode &other) { return this->value == other.value; }
