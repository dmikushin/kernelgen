  ...
  static void handler(void *instance) {
    ...
    do {
      ...
      for (list<Pass *>::iterator i = tracker->passes.begin(),
        ...
        if (!arg)
          continue;
        if (!strcmp(arg, "targetdata"))
          continue;
        if (!strcmp(arg, "targetpassconfig"))
          continue;
        if (!strcmp(arg, "machinemoduleinfo"))
          continue;
        D.addPass(arg);
...
public:
  PassTracker(const char *input, PassTrackerCallback callback,
              void *callback_arg)
      : input(input ? input : ""), callback(callback),
        callback_arg(callback_arg), module(NULL) {
  ...
  void run(Module *M) { module.reset(CloneModule(M)); }
};
...
public:
  TrackedPassManager(PassTracker *tracker) : PassManager(), tracker(tracker) {}
  ...
  ~TrackedPassManager() { tracker->reset(); }
};
