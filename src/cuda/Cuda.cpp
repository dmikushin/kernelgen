...
#define DL_SAFE_CALL(name, suffix)                                             \
  ...
  CU_SAFE_CALL(cuDeviceGetAttribute(
      &threadsPerBlock, CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK, device));

  // Get the regsPerBlock property.
  CU_SAFE_CALL(cuDeviceGetAttribute(
      &regsPerBlock, CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK, device));
...
unsigned int context::getLEPC() const {
  // Substract 0x10, because LEPC instruction comes as 3rd instruction on
  // Kepler.
  // TODO: 2nd instruction on Fermi.
  size_t szlepc = 4;
  unsigned int lepc;
  CU_SAFE_CALL(cuMemcpyDtoHAsync(&lepc, (CUdeviceptr) lepcBuffer, szlepc,
                                 secondaryStream));
