...
unsigned int
kernelgen::bind::cuda::CUBIN::InsertLEPCReporter(const char *cubin,
  ...
  try {
    ...
    if (elf_getshdrstrndx(e, &shstrndx))
      THROW("elf_getshdrstrndx() failed for " << cubin << ": "
                                              << elf_errmsg(-1));
    ...
    for (int i = 1; scn != NULL; scn = elf_nextscn(e, scn), i++) {
      ...
      if ((cuda_context->getSubarchMajor() == 2) &&
          (cuda_context->getSubarchMinor() == 0)) {
        ...
        size_t szbinary;
        char *binary = asfermi_encode_opcodes(
            source, cuda_context->getSubarchMajor() * 10 +
        ...
        free(binary);
      } else if (((cuda_context->getSubarchMajor() == 2) &&
                  (cuda_context->getSubarchMinor() > 0)) ||
      ...
      // Find a first occurrence of commands sequence being searched and replace
      // it.
      unsigned int lepc_offset = (unsigned int) - 1;
      for (int k = 0, ke = data->d_size - (search.size() - 1) * 8; k != ke;
           k += 8)
        if (!memcmp((char *)data->d_buf + k, (char *)&search[0],
                    search.size() * 8)) {
          memcpy((char *)data->d_buf + k, (char *)&replacement[0],
                 search.size() * 8);
          lepc_offset = k;
          break;
        }
      ...
      if (elf_flagdata(data, ELF_C_SET, ELF_F_DIRTY) == 0)
        THROW("elf_flagdata() failed for \"" << cubin
                                             << "\": " << elf_errmsg(-1));

      // Update ELF.
      if (elf_update(e, ELF_C_WRITE) == -1)
        THROW("elf_update() failed for \"" << cubin
                                           << "\": " << elf_errmsg(-1));
