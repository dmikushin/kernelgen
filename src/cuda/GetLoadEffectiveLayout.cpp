	...
	string kernel_name, unsigned int kernel_lepc, vector<char>& mcubin)
{
	...
	{
		uint64_t opcode = *(uint64_t*)&kernel_code[i];
		VERBOSE(Verbose::Loader << "0x" << opcode << "\n" << Verbose::Default);
	}
...
void kernelgen::bind::cuda::CUBIN::GetLoadEffectiveLayout(const char* cubin, const char* ckernel_name)
{
	// Read LEPC.
	...
	for (map<string, unsigned int>::iterator i = layout.begin(), ie = layout.end(); i != ie; i++)
	{
		...
		VERBOSE(Verbose::Loader << i->first << " -> 0x" << hexaddress << "\n" << Verbose::Default);
	}
}
