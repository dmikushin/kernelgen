//===- helperMixed.cpp - AsFermi mixed functionality ----------------------===//
//
...
void hpCleanUp() //parsers are created with the new keyword,        =====
  ...
  /*delete[] csInstructionRules;
 	...
 	for(list<Kernel>::iterator i = csKernelList.begin(); i != csKernelList.end();
 i++)
 	{
...
void hpUsage() {
  ...
  puts("Source file must be the first command-line option. However, it could "
       "be repla-");
  puts("ced by a -I option specified below.");
  puts("Options:");
  puts("  -I \"instruction\": This can be used to replace the inputfile.");
  puts("     A single line of instruction, surrounded by double quotation "
       "marks, will");
  puts("     be prrocessed as source input. Note that comment is not supported "
       "in thi-");
  puts("     s mode.");
  puts("  -o outputfile: Output cubin to the specified file.");
  puts("  -r target_cubin kernel_name offset: Replace the opcodes in specified "
       "locati-");
  puts("     on of a kernel in a specified cubin file with the assembled "
       "opcodes.");
  puts("  -sm_20: output cubin for architecture sm_20 (default)");
  ...
  puts("  -SelfDebug: throw unhandled exception when things go wrong. For "
       "debugging o-");
  puts("     f asfermi only.");
}
...
int hpHexCharToInt(char *str) {
  ...
  if (str[0] != '0' || !(str[1] == 'x' || str[1] == 'X') || (int) str[2] == 0)
  ...
  if (result == 0) //when result = 0, str must be 0x00000..
    ...
    for (int i = 2; i < 20; i++) {
      if (str[i] == 0) //stop if end is reached.
        break;
      if (str[i] !=
          '0')   //when result is zero, all digitis ought to be '0' as well.
...
void hpReadSource(char *path) //===
  ...
  csSource =
      new char[csSourceSize + 1]; //+1 to make space for ToCharArray or the like
...
void hpCheckOutputForReplace(char *path, char *kernelname,
                             char *replacepoint) //===
    {
  //open and check file
  fstream csOutput;
  ...
  if (fileSize < 100 || (int) buffer[0] != 0x7f || (int) buffer[1] != 0x45 ||
      (int) buffer[2] != 0x4c || (int) buffer[3] != 0x46) //ELF file identifier
    throw 6;           //file to be modified is invalid
                       //Reading the ELF header
  unsigned int SHTOffset = *((unsigned int *)(
      buffer + 0x20)); //section header table offset, stored at 0x20, length 4
  unsigned int SHsize = *((unsigned short *)(
      buffer + 0x2e)); //section header size, stored at 0x2e, length 2
  unsigned int SHcount =
      *((unsigned short *)(buffer + 0x30)); //section header count
  unsigned int SHStrIdx =
      *((unsigned short *)(buffer + 0x32)); //section header string table index
  unsigned int SHStrOffset = *((unsigned int *)(
      buffer + SHTOffset + SHsize * SHStrIdx +
      0x10)); //offset in file of the section header string table, 0x10 is the
              //offset in a header for section offset
  ...
  for (unsigned int i = 0; i < SHcount; i++, fileoffset += SHsize) {
    unsigned int SHNameIdx = *((unsigned int *)(
        buffer +
        fileoffset)); //first 4-byte word in a section header is the name index
...
void hpPrintInstructions() {
  ...
  for (list<Instruction>::iterator i = csInstructions.begin();
       i != csInstructions.end(); i++) {
...
void hpPrintDirectives() {
  ...
  for (list<Directive>::iterator i = csDirectives.begin();
       i != csDirectives.end(); i++) {
...
void hpPrintComponents(Instruction &instruction) {
  ...
  delete[] line;
  int count = 0;
  for (list<SubString>::iterator i = instruction.Components.begin();
       i != instruction.Components.end(); i++, count++) {
...
void hpBinaryStringToOpcode8(char *string, unsigned int &word0,
                             unsigned int &word1) {
  word0 = 0;
  int i = 0;
  hpBinaryStringToOpcode4(string, word0, i);
