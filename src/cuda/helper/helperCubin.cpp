//===- helperCubin.cpp - AsFermi CUBIN ELF manipulation functionality -----===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains various helper functions used during cubin output (the
...
void hpCubinStage1() {
  ...
  cubinCurrentSHStrTabOffset +=
      strlen(cubin_str_shstrtab) +
      1; //increment by length of name + length of the ending zero
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin(),
                              kernele = csKernelList.end();
       kernel != kernele; kernel++) {
    //Text
    hpCubinStage1SetSection(kernel->TextSection, KernelText,
                            kernel->KernelName.Length);
    //Constant0
    hpCubinStage1SetSection(kernel->Constant0Section, KernelConstant0,
                            kernel->KernelName.Length);
    //Info
    int infoSize = 12;
    if (kernel->Parameters.size() != 0)
      infoSize = 0x14 * (kernel->Parameters.size() + 1);
    hpCubinStage1SetSection(kernel->InfoSection, KernelInfo,
                            kernel->KernelName.Length);
    //Shared
    if (kernel->SharedSize != 0)
      hpCubinStage1SetSection(kernel->SharedSection, KernelShared,
                              kernel->KernelName.Length);
    //Local
    if (kernel->LocalSize != 0)
      hpCubinStage1SetSection(kernel->LocalSection, KernelLocal,
                              kernel->KernelName.Length);
  }
  ...
  cubinCurrentStrTabOffset =
      cubinCurrentSHStrTabOffset; //jump over shstr length being duplicated
  for (list<Kernel>::iterator kernel = csKernelList.begin(),
                              kernele = csKernelList.end();
       kernel != kernele; kernel++) {
    //StrTabOffset
  ...
  for (list<Constant2>::iterator constant2 = csConstant2List.begin(),
                                 constant2e = csConstant2List.end();
       constant2 != constant2e; constant2++) {
    //StrTabOffset
...
inline void hpCubinStage2SetSHStrTabSectionContent() {
  ...
  int currentOffset = 1;
  //head sections
  hpCubinAddSectionName2(cubinSectionSHStrTab.SectionContent, currentOffset,
                         cubin_str_shstrtab);
  hpCubinAddSectionName2(cubinSectionSHStrTab.SectionContent, currentOffset,
                         cubin_str_strtab);
  hpCubinAddSectionName2(cubinSectionSHStrTab.SectionContent, currentOffset,
  ...
  //kern sections
  for (list<Kernel>::iterator kernel = csKernelList.begin();
       kernel != csKernelList.end(); kernel++) {
    hpCubinAddSectionName1(cubinSectionSHStrTab.SectionContent, currentOffset,
                           cubin_str_text, kernel->KernelName);
    hpCubinAddSectionName1(cubinSectionSHStrTab.SectionContent, currentOffset,
                           cubin_str_constant0, kernel->KernelName);
    hpCubinAddSectionName1(cubinSectionSHStrTab.SectionContent, currentOffset,
                           cubin_str_info, kernel->KernelName);
    if (kernel->SharedSize)
      hpCubinAddSectionName1(cubinSectionSHStrTab.SectionContent, currentOffset,
                             cubin_str_shared, kernel->KernelName);
    if (kernel->LocalSize)
      hpCubinAddSectionName1(cubinSectionSHStrTab.SectionContent, currentOffset,
                             cubin_str_local, kernel->KernelName);
  }
  //tail sections
  if (cubinConstant2Size) {
    // Switch between .nv.constant2 and .nv.constant3, depending on target
    // architecture.
...
inline void hpCubinStage2SetStrTabSectionContent() {
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin(),
                              kernele = csKernelList.end();
       kernel != kernele; kernel++)
    hpCubinAddSectionName3(cubinSectionStrTab.SectionContent, currentOffset,
                           kernel->KernelName);
  ...
  for (list<Constant2>::iterator constant2 = csConstant2List.begin(),
                                 constant2e = csConstant2List.end();
       constant2 != constant2e; constant2++)
    hpCubinAddSectionName2(cubinSectionStrTab.SectionContent, currentOffset,
                           constant2->Constant2Name.c_str());
}

inline void hpCubinStage2AddSectionSymbol(ELFSection &section,
                                          ELFSymbolEntry &entry, int &index,
                                          unsigned int size) {
  entry.Reset();
...
inline void hpCubinStage2SetSymTabSectionContent() {
  ...
  int entryCount = cubinCurrentSectionIndex - 3 + csKernelList.size() +
                   csConstant2List.size();
  ...
  memset(cubinSectionSymTab.SectionContent, 0,
         cubinSectionSymTab.SectionSize); //clear everything to 0 first
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin();
       kernel != csKernelList.end(); kernel++) {
    //text
    hpCubinStage2AddSectionSymbol(kernel->TextSection, entry, index,
                                  kernel->TextSize);
    //constant0
...
void hpCubinStage2() {
  cubinSectionSHStrTab.SectionContent =
      new unsigned char[cubinCurrentSHStrTabOffset];
  cubinSectionStrTab.SectionContent =
      new unsigned char[cubinCurrentStrTabOffset];
...
void hpCubinStage3() {
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin();
       kernel != csKernelList.end(); kernel++) {
    //.text
    kernel->TextSection.SectionSize =
        kernel->TextSize; //device functions not implemented. When implemented,
                          //this has to include size of device func as well
    kernel->TextSection.SectionContent =
        new unsigned char[kernel->TextSection.SectionSize];
    unsigned int *offset = (unsigned int *)(kernel->TextSection.SectionContent);
    for (list<Instruction>::iterator inst = kernel->KernelInstructions.begin();
         inst != kernel->KernelInstructions.end(); inst++) {
    ...
    if (cubinArchitecture == sm_20 || cubinArchitecture == sm_21) {
      ...
      kernel->Constant0Section.SectionContent =
          new unsigned char[kernel->Constant0Section.SectionSize];
      memset(kernel->Constant0Section.SectionContent, 0,
             kernel->Constant0Section.SectionSize); //just set it all to 0
    } else if (cubinArchitecture == sm_30) {
    ...
    if (kernel->Parameters.size() == 0) //no param
      ...
      *offset++ = kernel->Constant0Section
          .SymbolIndex;       //next value is constant0 section symbol index
      *offset = 0x200000;     //without param, this is always 0x00200000
    } else {
      if (cubinArchitecture == sm_21) {
        kernel->InfoSection.SectionSize =
            0x14 * (kernel->Parameters.size() + 1); //size = (n+1)(0x14)
      } else if (cubinArchitecture == sm_20 || cubinArchitecture == sm_30) {
        kernel->InfoSection.SectionSize =
            0x10 * (kernel->Parameters.size() + 1); //size = (n+1)(0x14)
      }
      kernel->InfoSection.SectionContent =
          new unsigned char[kernel->InfoSection.SectionSize];
      offset = (unsigned int *)kernel->InfoSection.SectionContent;

      if (cubinArchitecture == sm_21) {
        //---cbank_param_offsets
        *offset++ =
            0x00000c04 | kernel->Parameters.size() * 4
                             << 16; //04 0c aa bb: bbaa is paramcount * 4
      ...
      *offset++ = kernel->Constant0Section.SymbolIndex;
      if (cubinArchitecture == sm_20 || cubinArchitecture == sm_21) {
        *offset++ = kernel->ParamTotalSize << 16 |
                    0x0020; //0x00aa0020: 0xaaaa: total parameter size
      } else if (cubinArchitecture == sm_30) {
        *offset++ = kernel->ParamTotalSize << 16 |
                    0x0140; //0x00aa0140: 0xaaaa: total parameter size
      }

      //---cbank_param_size
      *offset++ =
          0x00001903 |
          kernel->ParamTotalSize << 16; //03 19 aa bb: 0xbbaa: total param size

      //---kparam_info
      unsigned int ordinal =
          kernel->Parameters.size() - 1; //starts from the end of the param list
      for (list<KernelParameter>::reverse_iterator param =
               kernel->Parameters.rbegin();
           param != kernel->Parameters.rend(); param++) {
        *offset++ = 0x000c1704; //identifier: 04 17 0c 00
        *offset++ = 0x0;        //index, always -0x1
                                //*offset++ = 0xffffffff; //index, always -0x1
        *offset++ = (ordinal--) |
                    (param->Offset
                     << 16);    // aa bb cc dd: bbaa is ordinal, ddcc is offset
        *offset++ = (((param->Size + 3) / 4) << 20) |
                    0x0001f000; //aaa b c b dd: aaa is size of param/4, bb is
                                //cbank, c is space, dd is logAlignment
      }
    }
  ...
  //.nv.info
  cubinSectionNVInfo.SectionSize =
      0x18 *
      csKernelList
          .size(); //it's guaranteed by the caller that size is greater than 0
  cubinSectionNVInfo.SectionContent =
      new unsigned char[cubinSectionNVInfo.SectionSize];
...
void hpCubinStage4() {
  //unsigned int fileOffset = (cubin64Bit?0x40:0x34) + 0x28 *
  //cubinCurrentSectionIndex; //start of the shstrtab section content
  unsigned int fileOffset =
      ELFHeaderSize +
      ELFSectionHeaderSize *
          cubinCurrentSectionIndex; //start of the shstrtab section content
  ...
  cubinSectionSymTab.SectionHeader.Info =
      cubinCurrentSectionIndex + 2; //info is number of local symbols
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin();
    ...
    kernel->TextSection.SectionHeader.Flags =
        6 | kernel->BarCount << 20; //20:26 are bar count
    kernel->TextSection.SectionHeader.Link = 3;
    kernel->TextSection.SectionHeader.Info =
        kernel->TextSection.SymbolIndex |
        kernel->RegCount << 24; //highest byte is number of reg
                                //.constant0
    hpCubinSetELFSectionHeader1(kernel->Constant0Section, 1, 4, fileOffset);
    kernel->Constant0Section.SectionHeader.Flags = 2;
    kernel->Constant0Section.SectionHeader.Info =
        kernel->TextSection.SectionIndex;
    //.info
    hpCubinSetELFSectionHeader1(kernel->InfoSection,
                                0x70000000 /* SHT_LOPROC */, 1, fileOffset);
    ...
    if (kernel->SharedSize > 0) {
      ...
      kernel->SharedSection.SectionHeader.Info =
          kernel->TextSection.SectionIndex;
    }
    //.local
    if (kernel->LocalSize > 0) {
      ...
      kernel->LocalSection.SectionHeader.Info =
          kernel->TextSection.SectionIndex;
    }
  }
  //---tail sections
  ...
  //.nv.info
  hpCubinSetELFSectionHeader1(cubinSectionNVInfo, 0x70000000 /* SHT_LOPROC */,
                              1, fileOffset);
...
void hpCubinStage5() {
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin();
    ...
    kernel->KernelSegmentHeader.Offset =
        kernel->TextSection.SectionHeader.FileOffset;
    kernel->KernelSegmentHeader.FileSize =
        kernel->TextSize + kernel->Constant0Section.SectionSize +
        kernel->InfoSection.SectionSize;
    ...
    if (kernel->SharedSize != 0 || kernel->LocalSize != 0) {
      ...
      if (kernel->SharedSize != 0)
        kernel->MemorySegmentHeader.Offset =
            kernel->SharedSection.SectionHeader.FileOffset;
      else
        kernel->MemorySegmentHeader.Offset =
            kernel->LocalSection.SectionHeader.FileOffset;
      kernel->MemorySegmentHeader.FileSize = 0;
      kernel->MemorySegmentHeader.MemSize =
          kernel->SharedSize + kernel->LocalSize;
  ...
  if (cubinConstant2Size) {
    ...
    cubinSegmentHeaderConstant2.Offset =
        cubinSectionConstant2.SectionHeader.FileOffset;
...
void hpCubinStage7(iostream &csOutput) {
  ...
  //---Sections
  //head
  csOutput.write((char *)cubinSectionSHStrTab.SectionContent,
                 cubinSectionSHStrTab.SectionSize);
  csOutput.write((char *)cubinSectionStrTab.SectionContent,
                 cubinSectionStrTab.SectionSize);
  //..symtab
  if (!cubin64Bit)
    csOutput.write((char *)cubinSectionSymTab.SectionContent,
                   cubinSectionSymTab.SectionSize);
  else {
    int entryCount = cubinSectionSymTab.SectionSize / ELFSymbolEntrySize;
    ELFSymbolEntry *entries =
        (ELFSymbolEntry *)cubinSectionSymTab.SectionContent;
  ...
  for (list<Kernel>::iterator kernel = csKernelList.begin();
       kernel != csKernelList.end(); kernel++) {
    csOutput.write((char *)kernel->TextSection.SectionContent,
                   kernel->TextSection.SectionSize);
    csOutput.write((char *)kernel->Constant0Section.SectionContent,
                   kernel->Constant0Section.SectionSize);
    csOutput.write((char *)kernel->InfoSection.SectionContent,
                   kernel->InfoSection.SectionSize);
  ...
  //tail
  if (cubinConstant2Size)
    csOutput.write((char *)cubinSectionConstant2.SectionContent,
                   cubinSectionConstant2.SectionSize);
  csOutput.write((char *)cubinSectionNVInfo.SectionContent,
                 cubinSectionNVInfo.SectionSize);
