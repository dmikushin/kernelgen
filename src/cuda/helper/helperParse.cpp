//===- helperMixed.cpp - AsFermi parser functions -------------------------===//
//
	...
	while (true) {
		...
		for (; currentPos < directiveString.Length; currentPos++) {
			if (directiveString[currentPos] < 33) {
				csCurrentDirective.Parts
        .push_back(directiveString.SubStr(startPos, currentPos - startPos));
				currentPos++;
				startPos = currentPos;
				break;
			}
...
int b_startPos; //starting Position of a non-blank character in the instruction
                //string
int b_currentPos; //current search position in the instruction string. For the
                  //macros, this is the position of dot
int b_lineLength; //length of the instruction string

//b_startPos = b_currentPos = position of first non-blank character found in
//[b_currentPos, b_lineLength)
...
#define mExtract(startPos, cutPos)                                             \
  csCurrentInstruction.InstructionString.SubStr(startPos, cutPos - startPos)
#define mExtractPushComponent                                                  \
  {                                                                            \
    csCurrentInstruction.Components                                            \
        .push_back(mExtract(b_startPos, b_currentPos));                        \
  }
#define mExtractPushModifier                                                   \
  {                                                                            \
    if (b_currentPos == b_startPos) {                                          \
      throw 123;                                                               \
    }                                                                          \
    csCurrentInstruction.Modifiers                                             \
        .push_back(mExtract(b_startPos, b_currentPos));                        \
  }
...
static unsigned int predRef[] = { 0u, 1 << 10, 2 << 10, 3 << 10, 4 << 10,
                                  5 << 10, 6 << 10, 7 << 10 };
...
void hpParseProcessPredicate() {
	...
	if (negate)
   csCurrentInstruction.OpcodeWord0 |= predRefNegate;
}
