...
public :
	...
	Cubin(char* start, size_t size) : start(start), size(size), materialized(false)
	{
	}
	...
	int LaunchKernel(const char* kernelName,
		unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ,
		unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ,
		unsigned int sharedMemBytes, CUstream hStream, void* args, size_t szargs)
	{
		if (!materialized) Materialize();
		...
		CU_SAFE_CALL(cudyLoadCubinData((CUDYfunction*)&kernel_func,
			cuda_context->loader, kernelName, start, size,
			cuda_context->getSecondaryStream()));

		// Launch kernel function.
		// TODO: take care of szargs
		CU_SAFE_CALL(cudyLaunch((CUDYfunction)kernel_func, gridDimX,
				gridDimY, gridDimZ, blockDimX, blockDimY,
				blockDimZ, sharedMemBytes, args,
...
public :
	...
	void Merge(const vector<Cubin>& cubins)
	{
		...
		for (int i = 0, e = cubins.size(); i != e; i++)
		{
			...
			const vector<string>& kernelsNames = this->cubins[offset + i].getKernelsNames();
			for (int ii = 0, ee = kernelsNames.size(); ii != ee; ii++)
			{
				...
				this->operator[](kernelName)[real_cc][virt_cc] = &this->cubins[offset + i];
			}
		}
	}

	// Merge entire CUBINs index with another one.
	void Merge(const CubinsIndex& other)
	{
		Merge(other.cubins);
	}
...

	{
		...
		if (!cubins.size())
		{
			THROW("Cannot find CUBINs containing \"" << kernelName << "\" kernel for sm_" << real_cc);
		}
		Cubin* cubin = NULL;
		{
			...
			else
			{
				...
				int max_virt_cc = 0;
				for (map<char, Cubin*>::iterator i = cubins.begin(), e = cubins.end(); i != e; i++)
				{
		...
		return cubin->LaunchKernel(kernelName, gridDimX, gridDimY, gridDimZ,
			blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, args, szargs);
	}
};
...
public:
	...
	void Materialize()
	{
		...
		try
		{
			...
			{
				THROW("Cannot read ELF image from " << filename << ": " << elf_errmsg(-1));
			}
			...
			if (elf_getshdrstrndx(e, &shstrndx))
			{
				THROW("elf_getshdrstrndx() failed for " << filename << ": " << elf_errmsg(-1));
			}
			...
			for (int i = 1; scn != NULL; scn = elf_nextscn(e, scn), i++)
			{
				...
				GElf_Shdr shdr;
				if (!gelf_getshdr(scn, &shdr)) {
					THROW("gelf_getshdr() failed for " << filename << ": " << elf_errmsg(-1));
				}

				char* name = elf_strptr(e, shstrndx, shdr.sh_name);
				if (strcmp(name, ".nv_fatbin")) continue;
	...
	int thread_limit, uint3* tid, uint3* bid, dim3* bDim, dim3* gDim, int* wSize)
{
	...
	typedef int (*__cudaRegisterFunction_t)(
...
int cudaConfigureCall(dim3 gridDim, dim3 blockDim, size_t sharedMem, void* stream)
{
...
int cudaLaunch(char* entry)
{
	...
	return cubinsIndex.LaunchKernel(kernelName.c_str(),
		currKernelConfig.gridDim.x, currKernelConfig.gridDim.y, currKernelConfig.gridDim.z,
		currKernelConfig.blockDim.x, currKernelConfig.blockDim.y, currKernelConfig.blockDim.z,
		currKernelConfig.sharedMem, cuda_context->getSecondaryStream(),
		&currKernelConfig.arguments[0], szarguments);
}
