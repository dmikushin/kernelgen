...
extern "C" int cudaMalloc(void **ptr, size_t size) {
  ...
  if (!malloc_module) {
    ...
    malloc_kernel = kernelgen::runtime::Codegen(KERNELGEN_RUNMODE_CUDA, &malloc,
                                                malloc_module);
  ...
  // Pack call arguments.
  {
    ...
    args.size = size;
    CU_SAFE_CALL(
        cuMemcpyHtoDAsync(args_dev, &args, sizeof(cudaMalloc_t), stream));
  }
  ...
  CU_SAFE_CALL(
      cudyLaunch((CUDYfunction) malloc_kernel, gridDim.x, gridDim.y, gridDim.z,
  ...
  {
    CU_SAFE_CALL(
        cuMemcpyDtoHAsync(ptr, &args_dev->ptr, sizeof(void *), stream));
  }
...
extern "C" int cudaMemcpy(void *dst, void *src, size_t size, int kind) {
  ...
  case cudaMemcpyHostToDevice:
    CU_SAFE_CALL(
        cuMemcpyHtoDAsync(dst, src, size, cuda_context->getSecondaryStream()));
    return 0;
  case cudaMemcpyDeviceToHost:
    CU_SAFE_CALL(
        cuMemcpyDtoHAsync(dst, src, size, cuda_context->getSecondaryStream()));
    return 0;
  default:
