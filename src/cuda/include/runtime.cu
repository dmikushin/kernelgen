...
__attribute__((device)) __attribute__((always_inline)) int
kernelgen_posix_memalign(void **ptr, size_t alignment, size_t size) {
...
__attribute__((device)) __attribute__((always_inline)) void *
kernelgen_memalign(size_t boundary, size_t size) {
  // TODO: Do actual alignment somehow, currently
  // memory is always aligned to 4096 bytes.
...
__attribute__((device)) __attribute__((always_inline)) int
__iAtomicCAS(volatile int *p, int compare, int val) {
  int ret;
...
__attribute__((device)) __attribute__((always_inline)) void
kernelgen_hostcall(unsigned char *kernel, unsigned long long szdata,
  ...
  while (__iAtomicCAS(&callback->lock, 0, 0))
    continue;
}
...
__attribute__((device)) __attribute((always_inline)) int
kernelgen_launch(unsigned char *kernel, unsigned long long szdata,
                 unsigned long long szdatai, unsigned int *data) {
...
__attribute__((global)) void kernelgen_memcpy(void *dst, void *src,
                                              size_t size) {
