...
kernelgen_malloc_device(size_t size) {
  // Align size.
  if (size % 4096)
    size += 4096 - size % 4096;
  ...
  if (size + sizeof(kerelgen_memory_chunk_t) >
      km->szpool - (km->szused + km->count * sizeof(kerelgen_memory_chunk_t)))
    return NULL;

  // Find a free memory chunk.
  size_t i = 0;
  for (; i + size + sizeof(kerelgen_memory_chunk_t) < km->szpool;) {
    kerelgen_memory_chunk_t *p_mcb = (kerelgen_memory_chunk_t *)(km->pool + i);
