...
static void parse_elf_sections(int elfclass, const char *cubin, Elf *e,
                               int ielf, int shstrndx,
                               map<string, Elf_Datas> &sections,
                               map<string, Elf_Symbols> &symbols,
                               map<string, int> &strings, size_t &szdata,
                               size_t &sznames, size_t &szstrings,
                               size_t &nstrings, vector<string> &relocations,
    ...
    case SHT_REL:
      ...
      // If section is a relocation table:
      {
        if (!symtab_data) {
          fprintf(
              stderr,
              "Cannot parse relocations, since .symtab is not found for %s\n",
              cubin);
          throw;
        }
...
static void parse_elf_program_headers(int elfclass, const char *cubin, Elf *e,
                                      GElf_Ehdr &ehdr,
                                      map<string, Elf_Datas> &sections,
                                      vector<Elf_PHeader> &pheaders) {
  // Build a temporary "offset"-"section name" map.
  map<size_t, string> offsets;
  for (map<string, Elf_Datas>::iterator i = sections.begin(),
                                        ie = sections.end();
       i != ie; i++) {
  ...
  for (int i = 0; i < ehdr.e_phnum; i++) {
    ...
    if (offsets.find(phdr.p_offset) == offsets.end()) {
      fprintf(
          stderr,
          "PHeader refers an offset, where symbol could not be found: %zu\n",
...
static void parse_elf_symbols(const char *cubin1, Elf *e1, int shstrndx1,
                              const char *cubin2, Elf *e2, int shstrndx2,
                              int nsymbols, size_t &szsymbol,
                              map<string, Elf_Symbols> &symbols,
  ...
  symbols_data.resize(szsymbols);
  {
    ...
    int isym = 0;
    for (map<string, Elf_Symbols>::iterator i = symbols.begin(),
                                            ie = symbols.end();
         i != ie; i++, isym++) {
      ...
      if (syms.old_index[0] != -1) {
        ...
        fprintf(stderr,
                "Both symbols containers cannot be empty for symbol \"%s\"\n",
                i->first.c_str());
        throw;
      }
...
void kernelgen::bind::cuda::CUBIN::Merge(const char *input1, const char *input2,
                                         const char *output) {
  ...
  try {
    ...
    if (memcmp(&ehdr1.e_ident, &ehdr2.e_ident,
               sizeof(unsigned char) * EI_NIDENT)) {
    ...
    parse_elf_sections(elfclass, input1, e1, 0, shstrndx1, sections, symbols,
                       strings, szdata, sznames, szstrings, nstrings,
                       relocations, nvinfos);
    parse_elf_sections(elfclass, input2, e2, 1, shstrndx2, sections, symbols,
                       strings, szdata, sznames, szstrings, nstrings,
                       relocations, nvinfos);
    ...
    //
    size_t offset = 1 + strlen(".shstrtab") + 1 + strlen(".strtab") + 1 +
                    strlen(".symtab") + 1 + strlen(".nv.info") + 1;
    sznames += offset;
    ...
    for (map<string, int>::iterator i = strings.begin(), ie = strings.end();
         i != ie; i++) {
      const string &name = i->first;
    ...
    switch (elfclass) {
    case ELFCLASS32:
      parse_elf_symbols<Elf32_Sym>(input1, e1, shstrndx1, input2, e2, shstrndx2,
                                   nsymbols, szsymbol, symbols, symbols_data,
                                   sections);
      break;
    case ELFCLASS64:
      parse_elf_symbols<Elf64_Sym>(input1, e1, shstrndx1, input2, e2, shstrndx2,
                                   nsymbols, szsymbol, symbols, symbols_data,
                                   sections);
      break;
    default:
        ...
        for (int k = 0; k < nrelocs; k++) {
          ...
          case ELFCLASS32:
            ...
            rel.r_info = ELF32_R_INFO(
                symbols[relocations[k + relocations_offset]].new_index, type);
            break;
          case ELFCLASS64:
    ...
    for (int i = 0, k = 0, ie = pheaders.size(); i != ie; i++) {
      // TODO: Don't know what to do with 0 offset, that takes place sometimes.
      if (pheaders[i].phdr.p_offset == 0)
        continue;

      if (pheaders[i].phdr.p_type != PT_LOAD)
        continue;
