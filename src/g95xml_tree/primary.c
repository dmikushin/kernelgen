...
static match match_kind_param( int *kind, g95_expr **expr ) {
...
g95x_locus xwk;
    ...
    *expr = NULL;
    ...
    if( m != MATCH_NO ) {
        if( m == MATCH_YES ) {
            *expr = g95_int_expr( *kind );
            (*expr)->where = g95_current_locus;
            (*expr)->x.where = xwk;
        }
        return m;
    }
...
static int get_kind( g95x_kind **xkind ) {
int kind;
match m;
g95_expr *expr;
    ...
    m = match_kind_param( &kind, &expr );
    switch(m) {
    case MATCH_YES:
        if( kind >= 0 ) {
            (*xkind) = g95x_get_kind( NULL );
            (*xkind)->kind = expr;
            return kind;
        }
...
char cp;
    ...
    char* boz = g95_getmem( 1 + 1 + length + 1 + 1 );
    boz[0] = cp;
    boz[1] = '\'';
    memcpy( &boz[2], buffer, length );
    boz[length+2] = '\'';
    boz[length+3] = '\0';
    ...
    e->ts.x.kind = xkind;
    e->x.boz = boz;
    g95x_push_locus( &xw, G95X_LOCUS_BOZZ );
...
g95_expr *ekind = NULL;
	...
	kind = -1;
        ...
        ekind = g95_get_expr();
        ekind->type = EXPR_VARIABLE;
        ekind->where = g95_current_locus;
        ekind->symbol = sym;
        ekind->ts = sym->ts;
        ekind->x.where = xwk;
	...
	*result = g95_char_expr(kind, g95_default_character_kind(),
	...
	match_hollerith_constant(*result);
        (*result)->ts.x.kind = xkind;
        g95x_mark_2( &xw );
        g95x_push_locus( &xw, G95X_LOCUS_HLLR );
	return MATCH_YES;
    }
...
got_delim:
    ...
    g95x_mark_2( &xw );
    g95x_push_locus( &xw, G95X_LOCUS_STRG );
...
g95x_locus xw;
	...
	e->ref = g95_full_ref(&g95_def_locus);
        g95x_mark_1( &e->ref->x.where );
        g95x_mark_2( &e->ref->x.where );
	e->rank = sym->as->rank;
    }
...
g95x_locus xw, nxw;
    ...
    {
        nxw = g95x_pop_locus();
        g95x_push_locus( &nxw, G95X_LOCUS_KWRD ); /* so that keywords are not recognized
                                                    as names */
    }
...
g95x_locus xw;
	...
	if (g95_match_name(name) != MATCH_YES ||
		...
		goto cleanup;
            ...
            g95x_locus xe = g95x_pop_locus();
            g95x_locus xn = g95x_pop_locus();
      
            g95x_push_locus_mmbr( &xn, sym );
            g95x_push_locus( &xe, G95X_LOCUS_LANG );
	...
	if (m == MATCH_ERROR) goto cleanup;
        ...
        if( tail->x.where_kw.lb1 ) {
          tail->x.where.lb1     = tail->x.where_kw.lb1;
          tail->x.where.column1 = tail->x.where_kw.column1;
        }
	...
	for(tail=head; tail; tail=tail->next, comp=comp->next) {
		...
		(!comp->allocatable ||
		 (g95_option.fmode != 0 && g95_option.fmode <= 2003))) {
            ...
            if( comp->initializer->x.full )
                tail->expr = g95_copy_expr( comp->initializer->x.full );
            else
...
loop:
	...
	if (!array_flag && e->ts.type == BT_CHARACTER)
	    ...
	    ref = g95_extend_ref(e, REF_ARRAY, NULL);
            m = g95_match_array_ref(&ref->u.ar);
            if( m == MATCH_YES ) {
              g95x_mark_2( &xw );
              ref->x.where = xw;
            }
	...
	m = g95_match_name(name);
        ...
        if( m == MATCH_YES ) {
            ...
            g95x_push_locus_mmbr( &xw, e->ts.derived );
        }
        if( m == MATCH_YES ) {
            g95x_mark_2( &xw );
            ref->x.where = xw;
	...
	ref->u.ar.type = AR_FULL;
        ref->x.where.lb1 = ref->x.where.lb2 = xw.lb2;
        ref->x.where.column1 = ref->x.where.column2 = xw.column2;
    }
...
g95x_locus xw;
    ...
    if (g95_peek_char() == ':') {
	m = match_substring_rest(sym, &where, NULL, result, &xw);
	goto done;
    }
    ...
    m = match_substring_rest(sym, &where, a.u.expr, result, &xw);
...
g95x_locus xw, xwn;
    ...
    if (m == MATCH_YES) {
        ...
        if( (*result)->type == EXPR_FUNCTION ) {
            if( (*result)->value.function.pointer ) 
                (*result)->value.function.pointer->x.where = xwn;
        }
    }
...
static void fix_alloc_spec(g95_alloc *a, g95x_locus* xw) {
g95_ref *ref;
int i, rank;
	...
	ref->u.ar.dimen_type[i] = DIMEN_ELEMENT;
        ref->u.ar.x.section_where[i] = ref->u.ar.start[i]->x.where;
    }
}
