...
#define MODULE_EXTENSION ".gforscale.mod"
...
g95x_locus xw, xw_mod;
    ...
    g95x_mark_2( &xw_mod );
    g95x_module_set_mod( sym, &xw_mod );
	...
	case INTERFACE_GENERIC:
            ...
            new->x.use_name_el = g95x_get_current_ext_locus_ref();
            new->x.use_where = new->x.use_name_el->loc;
    
	    m = g95_match(" =>");
      
            if( m == MATCH_YES ) {
                new->x.local_name_el = new->x.use_name_el;
                new->x.use_name_el = NULL;
                new->x.local_where = new->x.local_name_el->loc;
            }
		...
		if (m != MATCH_YES)
		    ...
		    m = g95_match_name(new->use_name);
                    ...
                    if( m == MATCH_YES ) {
                        ...
                        g95x_push_locus( &xl, G95X_LOCUS_ASSC );
                    }
	...
	case INTERFACE_INTRINSIC_OP:
            ...
            new->x.use_name_el = g95x_get_current_ext_locus_ref() - 1;
            new->x.use_where = new->x.use_name_el->loc;
...
static void mio_component_list(g95_component **cp,g95_symbol *sym) {
...
static void mio_intf_info( g95x_intf_info *iif ) {
  int type;
  ...
  switch( iif->type ) {
    case INTERFACE_INTRINSIC_OP:
        mio_integer( &iif->u.iop_idx );
        break;
    case INTERFACE_GENERIC:
        mio_allocated_string( &iif->u.generic_name );
        break;
    case INTERFACE_USER_OP:
        mio_allocated_string( &iif->u.uop_name );
        break;
    case INTERFACE_NAMELESS:
        break;
    default:
        g95_internal_error( "Bad interface type" );
  }

  mio_allocated_string( &iif->module );
}
...
static void mio_interface_rest(g95_interface **ip,g95x_intf_info* iif) {
...
g95x_intf_info* ii;
	...
	if (ip != NULL)
		...
		if( q == p ) {
		    for( n = 0, ii = p->x.intf_info; ii; n++, ii = ii->next );
                    mio_integer( &n );
		    for( ii = p->x.intf_info; ii; ii = ii->next )
                      mio_intf_info( ii );
                }
	...
	for(;;) {
	    ...
	    ii = p->x.intf_info = g95x_get_intf_info();
            *(p->x.intf_info) = *iif;
            p->x.intf_info->next = NULL;

	    for( i = 0; i < n; i++ ) {
              ii->next = g95x_get_intf_info();
	      ii = ii->next;
              mio_intf_info( ii );
            }
...
static void mio_interface(g95_interface **ip,g95_intrinsic_op iop_idx) {
    g95x_intf_info iif, *piif;

    if( iomode == IO_INPUT ) {
      iif.type = INTERFACE_INTRINSIC_OP;
      iif.module = g95_get_string( &module_name[0] );
      iif.u.iop_idx = iop_idx;
      piif = &iif;
    } else {
      piif = NULL;
    }
 ...
 * to be private.  Component access is checked during searching */
    ...
    mio_component_list(&sym->components,sym);
...
/* Faire une liste de symtree */
	...
	for(;;) {
	    ...
	    uop = g95_get_uop(p);
            ...
            for( intf = uop->operator; intf && intf->next; intf = intf->next );

            g95x_intf_info iif;
            iif.type = INTERFACE_USER_OP;
            iif.module = g95_get_string( &module_name[0] );
            iif.u.uop_name = g95_get_string( p );

	    mio_interface_rest(&uop->operator,&iif);

	    set_module_locus(&m);

            intf = intf ? intf->next : uop->operator;

            set_local_name_locus_uop( p, uop, intf );
...
/* Faire une liste de symtree */
	    st = g95_get_generic(p, NULL);
            ...
            for( intf = st ? st->n.generic : NULL; intf && intf->next; intf = intf->next );

            g95x_intf_info iif;
            iif.type = INTERFACE_GENERIC;
            iif.module = g95_get_string( &module_name[0] );
            iif.u.generic_name = g95_get_string( atom_string );

	    mio_interface_rest(&st->n.generic,&iif);
	    set_module_locus(&m);

            intf = intf ? intf->next : st->n.generic;

            set_local_name_locus_gen( p, st, intf );
	...
	for(;;) {
		...
		if (sym == NULL) {
		    ...
		    sym->module = g95_get_string(info->u.rsym.module);
                    sym->x.module_name = g95_get_string( module_name );
...
/* Faire une liste de symtree */
		...
		info->u.rsym.referenced = 1;
            ...
            set_local_name_locus_sym( p, name, sym );
	...
	if (only_flag) {
	    ...
	    intf = &g95_current_ns->operator[i];
            while( *intf ) { 
              intf = &((*intf)->next);
	    }
	}

	mio_interface(&g95_current_ns->operator[i],i);

	if( only_flag ) {
          u->x.u_use.iop_intf = *intf;
        }
    }
...
pointer_info *p;
    ...
    if (check_access(sym->attr.access, sym->ns->default_access)) {
        ...
        if( sym->x.is_generic ) /* Artificial symbols should not be exported */
            goto end;
...
static void write_generic_interfaces(g95_symtree *st) {
	...
	mio_interface_rest(&st->n.generic,NULL);
    }
...
static char nullstring[] = "";
    ...
    mio_interface_rest(&uop->operator,NULL);
}
...
int i;
	...
	mio_interface(check_access(g95_current_ns->operator_access[i],
		      ...
		      ? &g95_current_ns->operator[i] : NULL,i);
    }
...
int rc;
	...
	if (st != NULL) {
	    ...
	    rc = 1;
            ...
            sym->x.imported = 1;
            sym->module = g95_get_string( (char*)g95x_current_intrinsic_module_name );
	}
    }
...
g95_state_data *p;
    ...
    module_fp = g95_open_included_file(filename,NULL);
    if (module_fp == NULL) {
