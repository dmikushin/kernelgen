...
static void module_path(const char *option) {
	...
	g95_fatal_error("Only one -fmod= or -J option allowed\n");

    if (option[0] == '\0')
	g95_fatal_error("Directory required after -fmod= or -J\n");
...
int i;
    ...
    if( ( i = g95x_parse_arg( argc, argv ) ) > 0 ) 
        return -i;
    ...
    if (strcmp(option, "-freal-loops") == 0 ) { 
	g95_option.real_loops = 1;
	return 1;
    }
	...
	module_path(option + 6);
	add_path(option + 6);
	return 1;
    }
    if (strncmp(option, "-J", 2) == 0) {
	module_path(option + 2);
	add_path(option + 2);
	return 1;
    }

    if( strncmp( option, "-D", 2 ) == 0 ) {
        g95_define_cpp_macro( (char*)option + 2, 1 );
        return 1;
    }

    if( strncmp( option, "-U", 2 ) == 0 ) {
        g95_define_cpp_macro( (char*)option + 2, 0 );
        return 1;
    }
