...
int flag, c;
	...
	g95_gobble_whitespace();
        ...
        case '!':
            do {
                if( g95_current_locus.column-2 < g95_current_locus.lb->size )
                  g95_current_locus.lb->mask[g95_current_locus.column-2] = G95X_CHAR_COMM;
	        c = g95_next_char();
            } while(c != '\n');
        ...
        case ';':
            g95_current_locus.lb->mask[g95_current_locus.column-2] = G95X_CHAR_SMCL;
            if (G95_STRICT_F())
	        break;
...
/* Apply the next character to the current possibilities */
    ...
    if( best_match != NULL ) {
        g95x_mark_2( &xw );
        g95x_push_locus( &xw, G95X_LOCUS_LANG );
    }
...
g95x_locus xw;
    ...
    g95x_mark_2( &xw );
    g95x_push_locus( &xw, G95X_LOCUS_NAME );
...
g95x_locus xw;
    ...
    g95x_mark_2( &xw );
    g95x_push_locus_inop( &xw, INTRINSIC_ASSIGN );
...
match m;
    ...
    g95x_get_last_locus( &new_st.x.label_where,  G95X_LOCUS_LABL, 2 );
    g95x_get_last_locus( &new_st.x.label2_where, G95X_LOCUS_LABL, 1 );
    g95x_get_last_locus( &new_st.x.label3_where, G95X_LOCUS_LABL, 0 );
...
g95x_locus xw;
    ...
    *c->next = new_st;
    ...
    c->next->x.where = xw;
    c->next->x.sta = ST_ASSIGNMENT;

    g95_clear_new_st();

    new_st.type = EXEC_WHERE;
    new_st.block = c;
    new_st.x.where = xw_where;
...
#define match(string, subr, statement) \
  if (g95_match(string) == MATCH_YES) { mm = subr(); sta = statement; goto got_match; }

    g95_clear_error();

    g95x_mark_1( &xw );
    match("all stop",     g95_match_all_stop,    ST_ALL_STOP)
 ...
 * in new_st.  Rearrange things so that the IF statement appears in new_st */
    ...
    *p = new_st;
    ...
    if( ( p->type == EXEC_WHERE ) 
     || ( p->type == EXEC_FORALL ) 
    ) {
        p->x.where.lb1 = xw.lb1;
        p->x.where.column1 = xw.column1;
    } else 
        p->x.where = xw;
...
/* Match an infinite DO, make it like a DO WHILE(.TRUE.) */
    ...
    g95x_get_last_locus( &xwlab, G95X_LOCUS_LABL, 0 );
...
g95x_locus xw;
	...
	sym->refs++;
        ...
        g95x_import_push_list_symbol( sym );
        g95x_import_set_locus( &xw );
...
g95x_locus xw, xwlab;
	...
	new_st.label = label;
        ...
        g95x_get_last_locus( &new_st.x.label_where, G95X_LOCUS_LABL, 0 );
...
/* g95_match_all_stop()-- Match the ALL STOP statement. */

match g95_match_all_stop(void) {
int stop_code;
g95_expr *e;

    if (match_stop_code(ST_ALL_STOP, &e, &stop_code) == MATCH_ERROR)
	return MATCH_ERROR;

    if (g95_pure(NULL, 0)) {
	g95_error("ALL STOP statement not allowed in PURE procedure at %C");
	g95_free_expr(e);
	return MATCH_ERROR;
    }

    new_st.type = EXEC_ALL_STOP;
    new_st.expr = e;
    new_st.ext.stop_code = stop_code;
...
g95x_locus xw;
	...
	if (g95_add_type(sym, &ts, NULL) == FAILURE)
        ...
        g95x_enumerator_push_list_symbol( sym );
...
g95x_locus xwlab, xwsym;
    ...
    g95x_get_last_locus( &xwsym, G95X_LOCUS_NAME, 0 );
...
match m;
	...
	if (head == NULL)
        ...
        tail->x.where = tail->expr->x.where;
...
g95x_locus xw;
	...
	if (g95_peek_char() == '%') {
	    if (g95_add_flavor(&sym->attr, FL_VARIABLE, name, NULL) == FAILURE)
		return FAILURE;
	...
	if (m == MATCH_ERROR)
      ...
      new_st.ext.sub.pointer->x.where = xw;
    ...
    if (new_st.ext.sub.pointer == NULL)
      new_st.x.symbol = sym;


    new_st.where = where;
    new_st.x.call_where = xw;
    return MATCH_YES;
    ...
    *tail = var;
	...
	m = g95_match_iterator(&parent->iter);
        if (m == MATCH_YES) {
          ...
          g95x_simplify_expr( &parent->iter.start );
          g95x_simplify_expr( &parent->iter.end );
          g95x_simplify_expr( &parent->iter.step );
          g95x_simplify_pop();
          break;
        }
	if (m == MATCH_ERROR) return MATCH_ERROR;
...
g95x_locus xw;
	...
	*result = expr;
        ...
        g95_expr *full = sym->value->x.full; /* must exist */
        sym->value->x.full = NULL;
        
	*result = g95_copy_expr(sym->value);
    
        sym->value->x.full = full;
        (*result)->x.full = g95_get_variable_expr( sym, &where );
        (*result)->x.full->x.where = xw;
...
match m;
    ...
    m = g95_match_name(name);
    g95x_locus xl = g95x_pop_locus();
    g95x_push_locus( &xl, G95X_LOCUS_CMMN );
...
g95x_locus xw;
	...
	m = match_common_name(name);
        g95x_get_last_locus( &xw, G95X_LOCUS_CMMN, 0 );
	if (m == MATCH_ERROR)
	    goto cleanup;
	...
	if (name[0] == '\0') {
		...
		*g = *h;
        ...
        g95x_common_push_list_common( h );
        g95x_common_set_locus( &xw );
...
/* Grab the list of symbols for this common. */
	...
	tail = sym;
        ...
        g95x_common_push_list_symbol( sym );
...
match m;
    ...
    g95x_get_last_locus( &g95_new_block->x.where, G95X_LOCUS_NAME, 0 );
...
match m;
    ...
    if (g95x_simplify_expr(&e) == FAILURE) {
	g95_free_expr(e);
	return FAILURE;
    }
...
g95x_locus xw, xwref;
	...
	*result = e;
	...
	ref = g95_extend_ref(e, REF_SUBSTRING, NULL);
        xref = g95_extend_ref( xe, REF_SUBSTRING, NULL );
	goto substring_end;
    }
	...
	ref->u.ar.where[0] = g95_current_locus;
        ...
        xref = g95_extend_ref( xe, REF_ARRAY, NULL );
        *xref = *ref;
        xref->u.ar.start[0] = f->x.full;
        f->x.full = NULL;
        xref->u.ar.x.section_where[0] = f->x.where;
 ...
 * comma, so we have a multidimensional array reference. */
	...
	if (match_equiv_ref_expr(&ref->u.ar.start[i]) == FAILURE)
        ...
        xref->u.ar.start[i] = ref->u.ar.start[i]->x.full;
        xref->u.ar.x.section_where[i] = xref->u.ar.start[i]->x.where;

        ref->u.ar.start[i]->x.full = NULL;
    
	g95_gobble_whitespace();
	ref->u.ar.dimen_type[i] = DIMEN_ELEMENT;

        xref->u.ar.dimen_type[i] = DIMEN_ELEMENT;
...
check_substring:
    ...
    if( xref ) {
      g95x_mark_2( &xwref );
      ref->x.where = xref->x.where = xwref;
    }
    ...
    xref->u.ss.start = ref->u.ss.start->x.full;
    ref->u.ss.start->x.full = NULL;
...
substring_end:
    if( g95_peek_char() != ')' ) {
        if( match_equiv_ref_expr( &ref->u.ss.end ) == FAILURE ) 
            goto cleanup;
      
        xref->u.ss.end = ref->u.ss.end->x.full;
        ref->u.ss.end->x.full = NULL;
    }
...
int flag;
    ...
    for(;;) {
        ...
        g95x_naml_push_list_symbol( group_name );
	...
	for(;;) {
		...
		goto error;
            ...
            g95x_naml_push_list_symbol( sym );
