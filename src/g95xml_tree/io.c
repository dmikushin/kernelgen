...
static match match_ltag(io_tag *tag, g95_st_label **label, g95x_locus *xw ) {
g95_st_label *old;
match m;
    ...
    g95x_get_last_locus( xw, G95X_LOCUS_LABL, 0 );
...
match m;
    ...
    m = match_ltag(&tag_err, &open->err, &open->x.err_where);
    if (m != MATCH_NO)
	return m;
...
match m;
    ...
    m = match_ltag(&tag_err, &close->err, &close->x.err_where);
    if (m != MATCH_NO)
	return m;
...
match m;
    ...
    m = match_out_tag(&tag_iostat, &fp->iostat);  if (m != MATCH_NO) return m;
    m = match_ltag(&tag_err, &fp->err, &fp->x.err_where);  if (m != MATCH_NO) return m;
...
g95x_locus xw;
	...
	dt->format_label = label;
        ...
        g95x_get_last_locus( &dt->x.format_label_where, G95X_LOCUS_LABL, 0 );
	...
	sym->attr.flavor == FL_NAMELIST) {
        g95x_get_last_locus( &dt->x.namelist_where, G95X_LOCUS_NAME, 0 );
	dt->namelist = sym;
	return MATCH_YES;
    }
...
match m;
    ...
    m = match_ltag(&tag_err, &dt->err, &dt->x.err_where);
    if (m == MATCH_YES)
    ...
    m = match_ltag(&tag_end, &dt->end, &dt->x.end_where);
    if (m == MATCH_YES)
    ...
    m = match_ltag(&tag_eor, &dt->eor, &dt->x.eor_where);
    if (m == MATCH_YES)
...
match m;
    ...
    m = match_ltag(&tag_err, &flush->err, &flush->x.err_where);
    if (m != MATCH_NO)
	return m;
...
match m;
    ...
    m = match_ltag(&tag_err, &inquire->err, &inquire->x.err_where);
    if (m != MATCH_NO)
	return m;
...
match m;
    ...
    m = match_ltag(&tag_err, &wait->err, &wait->x.err_where);
    if (m != MATCH_NO)
	return m;

    m = match_ltag(&tag_end, &wait->end, &wait->x.end_where);
    if (m != MATCH_NO)
	return m;

    m = match_ltag(&tag_eor, &wait->eor, &wait->x.eor_where);
    if (m != MATCH_NO)
	return m;
