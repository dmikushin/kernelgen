...
#define case_executable case ST_ALLOCATE: case ST_BACKSPACE: case ST_CALL: \
  ...
  case ST_SYNC_MEMORY: case ST_ALL_STOP: case ST_NOTIFY: case ST_QUERY
...
static g95x_locus current_statement_locus;

g95x_locus* g95x_get_current_statement_locus() {
  return &current_statement_locus;
}
	...
	if (sym->ts.type != BT_UNKNOWN)
	    ...
	    m = g95_match_type_spec(&sym->result->ts, 1);
            ...
            if( g95x_option.enable ) {
                g95x_statement *sta = g95_current_ns->x.statement_head;
                if( sta->type == ST_FUNCTION )
                    sta->u.function.ts = sym->result->ts;
            }
 ...
 * character. */
	...
	match("protected", g95_match_protected, ST_ATTR_DECL);
...
g95x_locus xw;
	...
	if (m != MATCH_YES || !g95_is_whitespace(g95_peek_char())) {
	    ...
	    while(isdigit(c));
            ...
            g95_statement_label->x.where = xw;
            g95x_push_locus_labl( &xw, g95_statement_label );
    ...
    g95x_mark_1( &current_statement_locus );

    sta = decode_statement();
  
    g95x_mark_2( &current_statement_locus );
...
char *p;
    ...
    case ST_ALL_STOP:       p = "ALL STOP";              break;
...
static g95_code *accept_statement( g95_statement st, g95_code *c ) {
g95_code *p;
    ...
    g95x_push_statement( st, p ? p : c, &current_statement_locus );
...
g95_state_data s;
    ...
    accept_statement( ST_DERIVED_DECL, NULL );
	...
	case ST_END_TYPE:
		...
		error_flag = 1;
            ...
            accept_statement( ST_END_TYPE, NULL );
	    break;
	...
	case ST_PRIVATE:
	    ...
	    s.sym->component_access = ACCESS_PRIVATE;
            accept_statement( ST_PRIVATE, NULL );
	    seen_private = 1;
	    break;
	...
	case ST_SEQUENCE:
	    ...
	    g95_add_sequence(&g95_current_block()->attr,
            ...
            g95x_push_statement( ST_SEQUENCE, NULL, &current_statement_locus );	       
...
g95_gsymbol *g;
    ...
    accept_statement( ST_INTERFACE, NULL );
...
loop:
	...
	g95_current_ns = current_interface.ns;
        g95x_push_statement( ST_END_INTERFACE, NULL, &current_statement_locus );     
	goto done;
...
decl:
    ...
    g95x_push_statement( st, NULL, &current_statement_locus );
...
loop:
	...
	default:
        ...
        /* Don't accept a statement twice */
        if( ( st != ST_INTERFACE ) 
         && ( st != ST_DERIVED_DECL ) 
         && ( st != ST_ENUM ) 
        ) accept_statement( st, NULL );
...
g95_state_data s;
    ...
    top = accept_statement(ST_CRITICAL,NULL);
...
int seen_else;
    ...
    seen_else = 0;
    top = accept_statement( ST_IF_BLOCK, NULL );
...
g95_code *top;
    ...
    top = accept_statement( ST_FORALL_BLOCK, NULL );
...
g95_statement st;
    ...
    top = accept_statement( ST_WHERE_BLOCK, NULL );
	...
	case ST_WHERE_BLOCK:
	    parse_where_block();
	    /* Fall through */
            break; /* Cannot understand the previous fall though, so added break */
...
g95_code *p;
    ...
    p = accept_statement( ST_SELECT_CASE, NULL );
...
g95_code *top;
    ...
    top = accept_statement( ST_DO, NULL );
...
int n;
    ...
    case ST_CONTAINS:
        g95x_push_statement( ST_CONTAINS, NULL, &current_statement_locus ); 
	goto contains;
...
g95_statement st;
    ...
    if( st == ST_END_BLOCK_DATA )  
        g95x_push_statement( ST_END_BLOCK_DATA, NULL, &current_statement_locus );    
}
...
*/
    ...
    if( g95x_option.enable )
        g95x_fixup_generics( g95_current_ns );
