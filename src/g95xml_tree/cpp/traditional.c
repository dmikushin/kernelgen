		      ...
		      pfile->out.cur = out_start;
                      if( ( ! pfile->state.in_directive ) 
                       && ( ! ( node->flags & NODE_BUILTIN ) ) 
                        ...
                        const uchar* text = macro1->exp.text;
                        size_t len = macro1->count;
                        g95x_cpp *xcp;

                        xcp = g95x_get_cpp( 1, len + 1 );
                        xcp->macro = macro1;
                        xcp->c1 = pfile->out.cur - pfile->out.base;
                        xcp->c2 = pfile->out.cur - pfile->out.base +
                          node->ident.len;
                        xcp->s[0].c1 = -1;
                        xcp->s[0].c2 = sizeof( g95x_cpp ) + 
                          2 * sizeof( g95x_cpp_item );
                        memcpy( (char*)xcp + xcp->s[0].c2, text, len );
...
replace_args_and_push (cpp_reader *pfile, struct fun_macro *fmacro)
{
  ...
  if (macro->paramc == 0)
  ...
  else
    {
      ...
      len0 = fmacro->args[fmacro->argc] - fmacro->args[0];
      ...
      for (exp = macro->exp.text;;)
	{
      ...
      p = BUFF_FRONT (buff);
      {
        ...
        xcp->c1 = pfile->out.cur - pfile->out.base;
        xcp->c2 = xcp->c1 + 
          fmacro->node->ident.len + 2 + len0 - 1;

      }		    
 
      st = sizeof( g95x_cpp ) + 2 * sizeof( g95x_cpp_item ) * sa;
      sa = 0;
      ...
      for (exp = macro->exp.text;;)
	{
	  ...
	  size_t arglen;
          ...
          if( b->text_len > 0 ) {
            ...
            xcp->s[sa].c1 = -1;
            xcp->s[sa].c2 = st;
            memcpy( (char*)xcp + st, b->text, b->text_len );
            st += b->text_len + 1;
            sa++;
          }
	  ...
	  arglen = (fmacro->args[b->arg_index]
		    - fmacro->args[b->arg_index - 1] - 1);
          {
            ...
            xcp->s[sa].c1 = fmacro->args[b->arg_index - 1] 
                          + fmacro->node->ident.len + 1;
            xcp->s[sa].c2 = fmacro->args[b->arg_index - 1] 
                          + fmacro->node->ident.len + 1 + arglen;
...
void g95x_print_macro( cpp_hashnode* hnode ) {
  ...
  printf( "<cpp_macro id=\"0x%x\" name=\"%s\"", macro, hnode->ident.str );

  
  if( macro->fun_like ) {
    printf( " args=\"" );
    for( i = 0; i < macro->paramc; i++ ) {
      printf( "0x%x", macro->params[i] );
      if( i+1 < macro->paramc )
        printf( "," );
    }
    printf( "\" value=\"" );
    ...
    for( exp = macro->exp.text; ; ) {
      ...
      if( b->text_len > 0 ) {
        if( b != (struct block *)macro->exp.text )
          printf( "," );
        printf( "0x%x", b->text );
      }
      ...
      printf( "0x%x", macro->params[b->arg_index-1] );
    ...
    printf( " value=\"0x%x\"", macro->exp.text );
  }
  ...
  if( macro->fun_like ) {
    ...
    for( i = 0; i < macro->paramc; i++ ) 
      printf( 
        "<cpp_macro_arg id=\"0x%x\" name=\"%s\"/>\n", 
        macro->params[i], 
        macro->params[i]->ident.str 
      );

    for( exp = macro->exp.text; ; ) {
      struct block *b = (struct block *)exp;
      if( b->text_len > 0 ) {
        printf( "<cpp_text id=\"0x%x\" value=\"" );
    ...
    printf( "<cpp_text id=\"0x%x\" value=\"", macro->exp.text );
    for( i = 0; macro->exp.text[i] != '\n'; i++ ) 
      printf( "%c", macro->exp.text[i] );
    printf( "\"/>\n" );
  }
