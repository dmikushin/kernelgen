...
int i;
    ...
    for(ar->dimen=0; ar->dimen<G95_MAX_DIMENSIONS; ar->dimen++) {
        g95x_mark_1( &ar->x.section_where[ar->dimen] );
	m = match_subscript(ar);
        g95x_mark_2( &ar->x.section_where[ar->dimen] );
	if (m == MATCH_ERROR)
	    goto error;
...
static match match_deferred_coarray_spec(g95_coarray_spec *cas) {
	...
	if (++cas->corank >= G95_MAX_DIMENSIONS) {
        ...
        g95x_mark_1( &cas->x.shape_spec_where[cas->corank-1] );

	if (g95_match_char(':') != MATCH_YES)
	    goto syntax;

        g95x_mark_2( &cas->x.shape_spec_where[cas->corank-1] );
    }
...
int i;
    ...
    for(;;) {
        g95x_mark_1( &cas->x.shape_spec_where[cas->corank] );
	if (g95_match_char('*') == MATCH_YES) {
            g95x_mark_2( &cas->x.shape_spec_where[cas->corank] );
            cas->x.assumed_where = cas->x.shape_spec_where[cas->corank];
	...
	if (g95_match_char(':') != MATCH_YES) {
	    ...
	    cas->lower[cas->corank] = g95_int_expr(1);
            ...
            g95x_mark_1( &cas->x.assumed_where );
	    if (g95_match_char('*') == MATCH_YES) {
                g95x_mark_2( &cas->x.assumed_where );
                g95x_mark_2( &cas->x.shape_spec_where[cas->corank] );
		cas->corank++;
		break;
	    }
...
array_type type;
    ...
    g95x_mark_1( &xw );
    g95x_mark_1( &as->x.shape_spec_where[as->rank-1] );

    if (g95_match_char('*') == MATCH_YES) {
        g95x_mark_2( &xw );
        as->x.assumed_where = xw;
    ...
    *upper = NULL;
    ...
    g95x_mark_2( &as->x.shape_spec_where[as->rank-1] );
    return type;
}
...
/* If a lower bounds of an assumed shape array is blank, put in one. */
    ...
    *asp = as;
    ...
    for(i=0; i<as->rank; i++)
	if (g95x_simplify_expr(&as->lower[i]) == FAILURE ||
	    g95x_simplify_expr(&as->upper[i]) == FAILURE)
	    m = MATCH_ERROR;
