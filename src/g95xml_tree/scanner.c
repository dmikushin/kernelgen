...
/* Character constants to be continued cannot have commentary after the '&' */
	...
	if (c != '!' && c != '\n') {
        ...
        if( old_loc.column-2 < old_loc.lb->size )
            old_loc.lb->mask[old_loc.column-2] = G95X_CHAR_CTFR;
 ...
 * the whitespace started and resume from there. */
	...
	if (c != '&') {
	    ...
	    bad_continue = 1;
            ...
            if( g95_current_locus.column-2 < g95_current_locus.lb->size )
                g95_current_locus.lb->mask[g95_current_locus.column-2] = G95X_CHAR_CTFR;
	}
...
/* See if this line is a continuation line */
	...
	c = next_char(0);
        ...
        if( c == '0' ) {
            if( g95_current_locus.column-2 < g95_current_locus.lb->size ) 
                g95_current_locus.lb->mask[g95_current_locus.column-2] = G95X_CHAR_ZERO;
        }
	...
	if (c == '0' || c == ' ')
        ...
        if( g95_current_locus.column-2 < g95_current_locus.lb->size ) 
            g95_current_locus.lb->mask[g95_current_locus.column-2] = G95X_CHAR_CTFX;
...
not_continuation:
    ...
    {  
        int size = g95_current_locus.lb->size;
        short* mask = g95_current_locus.lb->mask;
        char* line = g95_current_locus.lb->line;
        int k = g95_current_locus.column-2;
        if( in_string ) { 
            if( k < size )   
                mask[k] = G95X_CHAR_STRG;
            if( k > 0 && ( ( mask[k-1] == G95X_CHAR_NONE ) 
             || ( mask[k-1] == G95X_CHAR_CODE ) ) ) {
                if( k - 1 < size )
                    mask[k-1] = G95X_CHAR_STRG;
            }
        } else {
            if( k < size )
              if( ( mask[k] == G95X_CHAR_NONE ) 
               && ( line[k] != ' ' ) 
               && ( line[k] != '\t' ) 
               && ( line[k] != '\0' ) ) {
                mask[k] = G95X_CHAR_CODE;
              }
        } 
    } 
    continue_flag = 0;
...
int i;
    ...
    if (i == -1) {
        b = get_linebuf( strlen(line) );
	strcpy(b->line, line);

    } else {
        b = get_linebuf( 6 - i + strlen(line) );
...
int c, i;
	...
	buffer[i] = c;
        ...
        /* we load the full line, in the limits of G95_MAX_LINE */
	if (i >= G95_MAX_LINE-1 ) 
          g95x_die( "" );
...
g95_file *f;
    ...
    f->x.form = g95_current_form;
    f->x.width = line_width;
    f->x.include_level = include_level;
...
*/
    ...
    case LC_ENTER:
        ...
        current_file->line = g95x_current_line( current_file );  

	new_file((char *) map->to_file);

	int size = strlen( (char*)input->buffer->buf );
        current_file->x.input = input;
        current_file->x.text = g95_getmem( size + 1 );
        strcpy( current_file->x.text, (char*)input->buffer->buf );

        g95x_set_linetot (current_file);
...
char c;
	...
	input->out.base[len] = c;
        ...
        g95x_push_cpp_line( current_file, line_tail );
    }
}
...
g95_cpp_macro *p;
	...
	cpp_init_builtins(input, 1);
        ...
        for(r=g95x_builtin_macros; *r != NULL; r++)
...
char fullname[2*PATH_MAX];
	...
	if (fp == NULL) {
        ...
        fp = g95_open_included_file(name2,&fullname[0]);
	if (fp == NULL) {
	    g95_error_now("Can't open included file '%s'", name2);
	    goto done;
	}
        current_file->filename = g95_get_string( fullname );
    }

    struct stat st;
    fstat( fileno( fp ), &st );
    current_file->x.text = g95_getmem( st.st_size + 1 );
    fread( current_file->x.text, st.st_size, 1, fp );
    current_file->x.text[st.st_size] = '\0';
