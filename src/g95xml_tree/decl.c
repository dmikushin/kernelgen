...
static match char_len_param_value(g95_expr **expr,g95x_locus *xwc) {
match m;
g95x_locus xw;
	...
	*expr = NULL;
    ...
    if (g95x_simplify_spec_expr(expr) == FAILURE)
	return MATCH_ERROR;
...
static match match_char_length(g95_expr **expr,g95x_locus *xwc) {
int length;
match m;
g95x_locus xw;
	...
	*expr = g95_int_expr(length);
    ...
    m = char_len_param_value(expr,xwc);
...
/* Start updating the symbol table.  Add basic type attribute if present */
	...
	sym->ts.cl->length = g95_copy_expr(cl->length);
        sym->ts.cl->x.chst_where = cl->x.chst_where;
        if( cl->x.alias )
            sym->ts.cl->x.alias = cl->x.alias;
        else
            cl->x.alias = sym->ts.cl;
    }
 ...
 * dimension attribute. */
    ...
    g95x_decl_push_list_symbol( sym );
    set_dimension( current_as, as1, sym );
    set_length( current_ts.cl, cl1, sym );
...
static try init_symbol(char *name, g95_expr **initp, int do_set) {
g95_expr *e, *init;
g95_symbol *sym;
g95_charlen *cl;
int m;
...
/* Add initializer */
    ...
    sym->value = init;
    if( do_set )
      g95x_decl_set_initialization( sym );
    ...
    *initp = NULL;
	...
	if (!g95_compare_types(&sym->ts, &sym->value->ts)) {
	    ...
	    if (e != NULL) {
                e->x.full = sym->value->x.full;
                sym->value->x.full = NULL;
...
success:
    g95x_decl_push_list_component( c );
    exw = NULL;

    set_dimension( current_as, as1, c );
    set_length( current_ts.cl, cl, c );
...
g95x_locus xw, chst_where;
    ...
    if (m != MATCH_YES)
	goto cleanup;
    else if( g95_current_state() == COMP_DERIVED ) {
        g95x_locus xw = g95x_pop_locus();
        g95x_push_locus_mmbr( &xw, g95_new_block );
        exw = g95x_get_current_ext_locus_ref();
    }
	...
	as = g95_copy_array_spec(current_as);
        /*
         * Some explanation here: alias is the first matched
         * array spec; we have to do that, because we want the
         * original as to be printed out.
         * We do not have to take care of memory management too,
         * because the alias field is also directly referenced
         * by g95
         */
        if( current_as ) {
            if( current_as->x.alias == NULL )
                current_as->x.alias = (struct g95_array_spec*)as; 
            else
                as->x.alias = current_as->x.alias;
        }
	break;
	...
	switch(match_char_length(&char_len,&chst_where)) {
	case MATCH_YES:
	    if (G95_STRICT_F()) {
    ...
    if (m == MATCH_NO) {
        if( current_cas ) {
            if( current_cas->x.alias == NULL )
                current_cas->x.alias = (struct g95_coarray_spec*)cas; 
            else
                cas->x.alias = current_cas->x.alias;
        }
    }
 ...
 * Otherwise the statement is ambiguous with an assignment statement. */
	...
	if (g95_match(" =>") == MATCH_YES) {
	    ...
	    if (g95x_simplify_expr(&initializer) == FAILURE) {
		m = MATCH_ERROR;
		goto cleanup;
	    }
	...
	if (initializer != NULL) {
		...
		t = init_symbol(name, &initializer,1);
...
match m;
  ...
  /* Get the attributes, if any */
    ...
    ts.x.where = current_ts.x.where;
    ts.x.where_interface = current_ts.x.where_interface;
    ...
    current_ts = ts;
    current_attr.access     = access;
    current_attr.intent     = intent;
    current_attr.optional   = optional;
    current_attr.pointer    = pointer;
    current_attr.save       = save;
...
g95x_locus xw;
	...
	c->ts.interface = procedure_interface;
        ...
        c->ts.x.where = current_ts.x.where;
        c->ts.x.where_interface = current_ts.x.where_interface;
	...
	c->as          = NULL;
        ...
        current_attr.access       = c->access;
        current_attr.pointer      = c->pointer;
        current_attr.dimension    = c->dimension;
        current_attr.allocatable  = c->allocatable;
//        current_attr.nopass       = c->nopass;
...
match m;
    ...
    current_ts.x.where = *( g95x_get_current_statement_locus() );
	...
	if (g95_match_char(')') == MATCH_NO)
        ...
        g95x_get_last_locus( &current_ts.x.where_interface, G95X_LOCUS_NAME, 0 );
    ...
    g95x_mark_2( &current_ts.x.where );
...
/* Massage the kind numbers for complex types */
    ...
    if( g95x_simplify_none() ) {
        g95_expr* e = g95_int_expr( kind );
        e->x.where = xw;
        ts->x.kind = g95x_get_kind( NULL );
        ts->x.kind->kind = e;
    }
...
/* Try the old-style specification first */
    ...
    m = match_char_length(&len,&chst_where);
    if (m != MATCH_NO) {
...
/* Try the weird case:  ( KIND = <int> [ , LEN = <len-param> ] )   */
	...
	m = char_len_param_value(&len,&chst_where);
	if (m == MATCH_NO)     goto syntax;
	if (m == MATCH_ERROR)  goto done;
	seen_length = 1;
...
int c;
	...
	ts->type = BT_INTEGER;
	ts->kind = 1;  /* Hardwired */
        g95x_mark_2( &ts->x.where );
	return MATCH_YES;
    }

    if (g95_match(" character") == MATCH_YES) {
	ts->type = BT_CHARACTER;
	m = match_char_spec(ts, kind_flag);
	if( m == MATCH_YES )
          g95x_mark_2( &ts->x.where );
	return m;
    }
    ...
    g95x_get_last_locus( &ts->x.where_derived, G95X_LOCUS_NAME, 0 );
...
cleanup:
    ...
    if( ( m != MATCH_YES ) || ( ! g95x_option.enable ) ) {
        g95_free_coarray_spec(current_cas);
        g95_free_array_spec(current_as);
        current_cas = NULL;
        current_as = NULL;
...
try t;
	...
	p->sym = sym;
        ...
        g95x_get_last_locus( &p->x.where, sym ? G95X_LOCUS_NAME : G95X_LOCUS_LANG, 0 );
...
match m;
    ...
    *result = r;
    ...
    g95x_get_last_locus( &r->x.where, G95X_LOCUS_NAME, 0 );
...
int i;
    ...
    g95x_get_last_locus( &sym->x.where, G95X_LOCUS_NAME, 0 );
...
match m;
    ...
    g95x_get_last_locus( &entry->x.where, G95X_LOCUS_NAME, 0 );
...
g95_symbol *sym;
	...
	sym = g95_state_stack->previous->sym == NULL ? NULL
	    : g95_state_stack->previous->sym;
    } else {
      sym = g95_state_stack->sym;
    }
...
/* We haven't hit the end of statement, so what is left must be an end-name */
    ...
    if( sym ) 
      g95x_get_last_locus( &sym->x.end_where, G95X_LOCUS_NAME, 0 );
...
g95x_locus xw;
	...
	if (m == MATCH_ERROR) return MATCH_ERROR;
        ...
        g95x_get_last_locus( &xw, G95X_LOCUS_NAME | G95X_LOCUS_USOP | G95X_LOCUS_INOP, 0 );
	...
	case INTERFACE_INTRINSIC_OP:
		...
		goto done;
            ...
            g95x_decl_push_list_intr_op_idx( operator );
	...
	case INTERFACE_USER_OP:
		...
		goto done;
            ...
            g95_symtree *st = g95_find_symtree( g95_current_ns->uop_root, name );
      
            g95x_decl_push_list_user_op( st );
...
match m;
    ...
    g95_clear_attr(&current_attr);
    current_attr.access = ACCESS_PUBLIC;
...
match m;
	...
	m = g95_match(" / %n /", &name);
	if (m == MATCH_ERROR) goto end;
	if (m == MATCH_NO) goto syntax;

        g95x_ext_locus* exw = g95x_get_current_ext_locus_ref() - 1;

	h = g95_get_common(name, &g95_def_locus);
	h->saved = 1;
	g95_current_ns->seen_save = 1;

        g95x_decl_push_list_common( h );
        exw->type = G95X_LOCUS_CMMN;
...
g95x_locus xw;
    ...
    current_attr.flavor = FL_PARAMETER;
    if (init_symbol(sym->name, &init,0) == FAILURE)
	return MATCH_ERROR;
