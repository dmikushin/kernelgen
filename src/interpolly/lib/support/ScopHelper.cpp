...
Loop *kernelgen::castToLoop(const Region &R, LoopInfo &LI) {
...
Value *kernelgen::getPointerOperand(Instruction &Inst) {
...
bool kernelgen::isInvariant(const SCEV *S, Region &R) {
  return InvariantChecker::isInvariantInRegion(S, R);
}

// Helper function to check parameter
bool kernelgen::isParameter(const SCEV *Var, Region &RefRegion,
...
bool kernelgen::isIndVar(const SCEV *Var, Region &RefRegion,
                     LoopInfo &LI, ScalarEvolution &SE) {
...
bool kernelgen::isIndVar(const Instruction *I, const LoopInfo *LI) {
  Loop *L = LI->getLoopFor(I->getParent());

  return L && I == L->getCanonicalInductionVariable();
}

bool kernelgen::hasInvokeEdge(const PHINode *PN) {
...
BasicBlock *kernelgen::createSingleExitEdge(Region *R, Pass *P) {
...
void kernelgen::splitEntryBlockForAlloca(BasicBlock *EntryBlock, Pass *P) {
