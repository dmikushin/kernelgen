...
public:
  ...
  inline void interpret(const clast_block *e) {
    if (e->body) {
      std::map<const char *, int64_t> tmpClastNames(
          availableClastNames->begin(), availableClastNames->end());

      interpret(e->body);

      if (tmpClastNames.size() != availableClastNames->size()) {
        vector<const char *> unavailableNames;
        for (NameIterator name = availableClastNames->begin(),
                          end = availableClastNames->end();
             name != end; name++)
          if (tmpClastNames.find(name->first) != end)
            unavailableNames.push_back(name->first);

        for (vector<const char *>::iterator
                 unavailableName = unavailableNames.begin(),
                 end = unavailableNames.end();
             unavailableName != end; unavailableName++)
          availableClastNames->erase(*unavailableName);
      }
    }
  }
  inline void interpret(const clast_user_stmt *e) { return; }
...
public:
  ...
  SizeOfLoops(vector<Size3> *memForSizes = 0,
              bool *_isThereAtLeastOneParallelLoop = NULL)
      : sizeOfLoops(memForSizes), ScopPass(ID),
        isThereAtLeastOneParallelLoop(_isThereAtLeastOneParallelLoop) {}
...
bool isaGoodListOfStatements(const clast_stmt *stmt,
                             const clast_for *&nested_for,
                             bool &user_or_assignment) {
  // возвращает true, если
  //     в теле нет вложенных циклов
  //     в теле есть всего один вложенный цикл и
  // нет assignment или user_stmt, не вложенных в него
  // через nested_for возвращает указатель на
  // вложенный цикл или NULL если его нет
  ...
  if (CLAST_STMT_IS_A(stmt, stmt_user) || CLAST_STMT_IS_A(stmt, stmt_ass))
    if (nested_for)
      return false; // уже где-то есть цикл ---------------
    else
      user_or_assignment = true; // Нашли user/assignment !!!!!!!!!!!
  if (CLAST_STMT_IS_A(stmt, stmt_guard))
    good = isaGoodListOfStatements(((const clast_guard *)stmt)->then,
                                   nested_for, user_or_assignment);
  if (CLAST_STMT_IS_A(stmt, stmt_block))
    good = isaGoodListOfStatements(((const clast_block *)stmt)->body,
                                   nested_for, user_or_assignment);
  if (CLAST_STMT_IS_A(stmt, stmt_for))
    if (nested_for || user_or_assignment)
      return false; // уже где-то есть user/assignment или
                    // цикл --------
    else
      nested_for = (const clast_for *)stmt; //нашли for !!!!!!!!

  return good &&
         isaGoodListOfStatements(stmt->next, nested_for, user_or_assignment);
}
    ...
    if (isaGoodListOfStatements(stmt, nested_for, user_or_assignment)) {
      if (nested_for)                        // если есть цикл
        if (DP->isParallelFor(nested_for)) { // и он параллельный
          ...
          stmt = nested_for->body; //и проверяем тело
                                   //обнаруженного цикла
        } else
          break; //если встретился не параллельный
                 //цикл
      else
        break; // если нет больше циклов
    } else
      break; // если список не удовлетворяет
             // требованиям
  }
  return goodLoops;
}
...
Size3 SizeOfLoops::retrieveSize3FromCloogLoopAST(const clast_root *CloogAST,
                                                 int numLoops) {
...
bool SizeOfLoops::runOnScop(Scop &scop) {
  ...
  assert(sizeOfLoops->size() <= 1 && "only one scop allowed!");

  CloogInfo &C = getAnalysis<CloogInfo>();
  DP = &getAnalysis<Dependences>();
  const clast_root *root = C.getClast();

  //if there are some not substituted parameters then we can not compute size of
  //loops
  assert(scop.getNumParams() == 0 && "FIXME: "
                                     "After Constant Substitution number of "
                                     "scop's global parameters must be zero");
  ///   Those parameters are scalar integer values, which are constant during
  ///   execution.
  assert(
      root->names->nb_parameters == 0 &&
      "FIXME: "
  ...
  if (goodLoopsCount != 0)
    sizeOfLoops->push_back(retrieveSize3FromCloogLoopAST(root, goodLoopsCount));
  else
...
static RegisterPass<SizeOfLoops> Z("Size3-scop",
                                   "Compute Size3 structure for scops");
