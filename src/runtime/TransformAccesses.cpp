...
public:
  ...
  static int getAccessFunction(__isl_take isl_set *Set, __isl_take isl_aff *Aff,
                               void *User) {
    assert((*((isl_aff **)User) == NULL) &&
...
void copyIslAffToConstraint(isl_aff *affToCopy, isl_constraint *constraint) {
  ...
  assert(isl_aff_dim(affToCopy, isl_dim_in) == (int)
         isl_constraint_dim(constraint, isl_dim_in));
...
bool TransformAccesses::runOnScop(Scop &scop) {
  ...
  /*assert(scop.getNumParams() == 0 &&
 	       "FIXME: "
 	       "After Constant Substitution number of scop's global parameters must be
 zero"
 	       "if there are parameters then outer loop does not parsed");*/

  if (scop.getNumParams() != 0) {
    VERBOSE(Verbose::Polly << Verbose::Red
                           << "\n    FAIL: Scop has parameters, maybe not all "
                              "kernel detected as scop!!!\n" << Verbose::Reset
                           << Verbose::Default);
  ...
  for (Scop::iterator stmt_iterator = S->begin(), stmt_iterator_end = S->end();
    ...
    for (ScopStmt::memacc_iterator access_iterator = stmt->memacc_begin(),
                                   access_iterator_end = stmt->memacc_end();
      ...
      if (isa<AllocaInst>(*baseAddressValue)) {
        VERBOSE(Verbose::Polly
                << "MemoryAccess to alloca: " << *baseAddressValue << "\n    "
        ...
        memoryAccess->setCurrentRelationType(
            MemoryAccess::RelationType_general);
        isl_space_free(space);
        isl_map_free(accessRelation);
      } else {
        VERBOSE(Verbose::Polly
                << "MemoryAccess to pointer: " << *baseAddressValue << "\n    "
                << memoryAccess->getAccessRelationStr() << "\n        "
                << "allocSize: " << allocSize << " storeSize: " << storeSize
                << "\n" << Verbose::Default);
        ...
        if (!isa<ConstantExpr>(*baseAddressValue)) {
          VERBOSE(Verbose::Polly
                  << Verbose::Red
                  << "\n    FAIL: Scop contains indirect addressing, can not "
                     "compute access conflicts!!!\n" << Verbose::Reset
                  << Verbose::Default);
          isl_space_free(space);
          isl_map_free(accessRelation);
          getAnalysis<ScopInfo>().releaseMemory();
        ...
        ConstantInt *baseAddressConstant =
            cast<ConstantInt>(expr->getOperand(0));
        ...
        assert(accessFunction && "isl_pw_aff is empty?!");
        ...
        assert(isl_local_space_dim(affSpace, isl_dim_in) ==
               isl_local_space_dim(localSpace, isl_dim_in));
        assert(isl_local_space_dim(localSpace, isl_dim_out) == 1);

        isl_aff *multiplier = isl_aff_zero_on_domain(
            isl_aff_get_domain_local_space(accessFunction));
  ...
  /*// Iterate through all SCoP statements and find independent
 	...
 	for (Scop::iterator StmtI1 = S->begin(), StmtE1 = S->end(); StmtI1 != StmtE1;
 StmtI1++)
 	{
 			...
 			MAE1 = Stmt->memacc_end(); (MAI1 != MAE1) && IndependentMA; MAI1++)
 		{
 			...
 			for (Scop::iterator StmtI2 = S->begin(), StmtE2 = S->end(); StmtI2 !=
 StmtE2; StmtI2++)
 			{
...
Pass *createTransformAccessesPass() { return new TransformAccesses(); }
namespace llvm {
class PassRegistry;
void initializeTransformAccessesPass(llvm::PassRegistry &);
}
INITIALIZE_PASS_BEGIN(TransformAccesses, "transform-accesses",
                      "kernelgen's runtime trnasform accesses to general form",
                      false, false)
...
static RegisterPass<TransformAccesses>
    D("TransformAccesses",
      "Kernelgen - Transformation of accesses to general form");
