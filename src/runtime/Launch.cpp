...
int kernelgen_launch(Kernel *kernel, unsigned long long szdata,
  ...
  if (szdatai && (kernel->name != "__kernelgen_main")) {
    ...
    case KERNELGEN_RUNMODE_CUDA: {
      ...
      size_t size =
          (settings.getVerboseMode() != Verbose::Disable) ? szdata : szdatai;
      vector<char> vcontent;
      vcontent.resize(size);
      char *content = &vcontent[0];
      CU_SAFE_CALL(
          cuMemcpyDtoHAsync(content, &data->args, size, monitor_stream));
  ...
  case KERNELGEN_RUNMODE_NATIVE: {
    ...
    VERBOSE(Verbose::Perf << kernel->name << " time = " << t.get_elapsed()
                          << " sec\n" << Verbose::Default);
    break;
  }
  case KERNELGEN_RUNMODE_CUDA: {
    ...
    if (kernel->name != "__kernelgen_main") {
      ...
      VERBOSE(Verbose::Always
              << Verbose::Cyan << "Launching kernel " << kernel->name << "\n"
              << "    blockDim = " << blockDim << "\n"
              << "    gridDim = " << gridDim << "\n" << Verbose::Reset
              << Verbose::Flush);
      timer t;
      ...
      size_t szshmem = 0;
      CU_SAFE_CALL(cudyLaunch(
          (CUDYfunction) kernel_func, gridDim.x, gridDim.y, gridDim.z,
          blockDim.x, blockDim.y, blockDim.z, szshmem, &data,
          cuda_context->getSecondaryStream(), &only_kernel_time));
      ...
      VERBOSE(Verbose::Always << Verbose::Cyan << "Finishing kernel "
                              << kernel->name << "\n" << Verbose::Reset
                              << Verbose::Default << Verbose::Flush);

      VERBOSE(Verbose::Perf
              << kernel->name << " time = " << time << " sec\n" << kernel->name
              << " only kernel time = " << only_kernel_time << " sec\n"
              << Verbose::Default);
      break;
    }

    // Create host-pinned callback structure buffer.
    // Note buffer is intentionally placed onto heap, otherwise
    // Valgrind may have problems updating the buffer data.
    vector<struct kernelgen_callback_t> vcallback;
    vcallback.resize(1);
    struct kernelgen_callback_t *callback = &vcallback[0];
    CU_SAFE_CALL(
        cuMemHostRegister(callback, sizeof(struct kernelgen_callback_t), 0));
    ...
    // Launch main GPU kernel.
    {
      ...
      void *params[] = { CU_LAUNCH_PARAM_BUFFER_POINTER, &vargs[0],
                         CU_LAUNCH_PARAM_BUFFER_SIZE, &szvargs,
                         CU_LAUNCH_PARAM_END };
      CU_SAFE_CALL(
          cuLaunchKernel((void *)kernel_func, gridDim.x, gridDim.y, gridDim.z,
    ...
    // Launch monitor GPU kernel.
    {
      ...
      CU_SAFE_CALL(
          cudyLaunch((CUDYfunction) monitor_kernel, gridDim.x, gridDim.y,
                     gridDim.z, blockDim.x, blockDim.y, blockDim.z, szshmem,
                     args, cuda_context->getSecondaryStream(), NULL));
    }
    ...
    while (1) {
      ...
      // check the state.
      CU_SAFE_CALL(cuMemcpyDtoHAsync(callback, kernel->target[RUNMODE].callback,
                                     sizeof(struct kernelgen_callback_t),
                                     cuda_context->getSecondaryStream()));
      CU_SAFE_CALL(cuStreamSynchronize(cuda_context->getSecondaryStream()));
      /*printf("lock = %d, state = %d, kernel = %p, szdata = %d, szdatai = %d,
   data = %p\n",
   				callback->lock, callback->state, callback->kernel,
   				callback->szdata, callback->szdatai, callback->data);*/
      ...
      case KERNELGEN_STATE_LOOPCALL: {
        ...
        if (!callback->kernel->target[RUNMODE].supported) {
          ...
          kernelgen_hostcall(callback->kernel, FunctionTy, StructTy, &data);
          VERBOSE(Verbose::Perf << callback->kernel->name
                                << " time = " << t.get_elapsed() << " sec\n"
                                << Verbose::Default);
          break;
        }
        ...
        CU_SAFE_CALL(
            cuMemcpyHtoDAsync(&kernel->target[RUNMODE].callback->state, &state,
      ...
      case KERNELGEN_STATE_HOSTCALL: {
        ...
        timer t;
        {
          kernelgen_hostcall(callback->kernel, data->FunctionTy, data->StructTy,
                             data);
        }
        VERBOSE(Verbose::Perf << callback->kernel->name
                              << " time = " << t.get_elapsed() << " sec\n"
