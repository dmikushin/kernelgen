...
static void runPolly(kernel_t *kernel, vector<unsigned> &sizeOfLoops, bool mode, bool *isThereAtLeastOneParallelLoop)
{
	assert(kernel->name != "__kernelgen_main");

	{
		PassManager polly;
		polly.add(new TargetData(kernel->module));
		registerPollyPreoptPasses(polly);
		polly.add(llvm::createCFGSimplificationPass());
		polly.add(llvm::createLoopSimplifyPass());
		polly.add(new KernelPrepare());
		polly.add(polly::createIndependentBlocksPass());
	...
	kernelgen::parsingType = kernelgen :: InterprocedureParsing;
	{
		Function* rootFunction = kernel->module->getFunction(kernel->name);
		llvm::FunctionPassManager analysis(kernel->module);
		analysis.add(new TargetData(kernel->module));
		//analysis.add(createScopDescriptionPass());   // print scop description
		analysis.add(createTransformAccessesPass()); // create General Form for each scop's memory Access
		// set their current relation types to RelationType_general
		//analysis.add(createScopDescriptionPass());
		analysis.add(createCalculateParallelLoopsPass(&sizeOfLoops, isThereAtLeastOneParallelLoop));
		// Determine count of good nested parallel loops
		// Dependences analysis use general form of memory accesses
		// compute size of loops
		// determine is there exists at least one parallel loop
		analysis.add(createSetRelationTypePass()); // set current relation types in scop's memory Accesses back to
		// RelationType_polly
		//analysis.add(createScopDescriptionPass());
		analysis.run(*rootFunction);
	}

	kernelgen::CodeGenForCuda = mode;
	kernelgen::parsingType = kernelgen :: CodegenParsing;
	kernelgen::ClastStmtCodeGen::totalNumberOfParallelLoops = sizeOfLoops.size();

	if(!sizeOfLoops.empty())
		kernelgen::KernelCodeGenerator::addCUDADeclarations(kernel->module);

	{
		llvm::PassManager codegen;
		codegen.add(new TargetData(kernel->module));
		codegen.add(createKernelCodeGeneratorPass());
		//codegen.add(createCFGSimplificationPass());
		codegen.run(*kernel->module);
	}
	{
		std::list<Function *> functionsToErase;
		for(Module::iterator func = kernel->module->begin(), funcEnd = kernel->module->end();
		    func != funcEnd; func ++) {
			if(kernel->module->getNamedMetadata(func->getName()) && func->getNumUses() == 0)
				functionsToErase.push_back(func);
		}
		for(std::list<Function *>::iterator iter = functionsToErase.begin(), iterEnd = functionsToErase.end();
		    iter != iterEnd; iter++) {
			Function *func = *iter;
			const StringRef & name = func->getName();
			Function *newFunction = kernel->module->getFunction( name.str() + "_generated");
	...
	if (verbose & KERNELGEN_VERBOSE_POLLYGEN) {
		...
		statisticsNames.push_back("kernel-verification");
	...
	if(sizeOfLoops.empty() && (verbose & KERNELGEN_VERBOSE_POLLYGEN) ) {
...
static void runPollyNATIVE(kernel_t *kernel, vector<unsigned> &sizeOfLoops)
{
	return runPolly(kernel, sizeOfLoops, false, NULL);
}
static void runPollyCUDA(kernel_t *kernel, vector<unsigned> &sizeOfLoops, bool *isThereAtLeastOneParallelLoop)
{
	...
	switch (runmode) {
		case KERNELGEN_RUNMODE_NATIVE : {
			if (kernel->name != "__kernelgen_main") {
				...
				if (runmode == kernelgen::runmode) {
					...
					std::vector<unsigned> sizeOfLoops;
		...
		case KERNELGEN_RUNMODE_CUDA : {
			...
			for (Module::iterator func = m->begin(), funce = m->end(); func != funce; func++) {
				func->setCallingConv(CallingConv::PTX_Device);
...
#define BLOCK_DIM_X 32
				...
				int numberOfLoops = sizeOfLoops.size();
				sizeOfLoops.insert(sizeOfLoops.begin(), 3 - numberOfLoops, 1);
				Size3 launchParameters(sizeOfLoops[2], sizeOfLoops[1], sizeOfLoops[0]);
				...
				if (launchParameters.x * launchParameters.y * launchParameters.z > props.maxThreadsPerBlock)
					switch (numberOfLoops) {
						case 0:
							...
							break;
						case 1:
							blockDim = dim3(props.maxThreadsPerBlock, 1, 1);
							break;
						case 2:
							blockDim = dim3(BLOCK_DIM_X, props.maxThreadsPerBlock / BLOCK_DIM_X, 1);
							break;
						case 3: {
