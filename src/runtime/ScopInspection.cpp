...
public:
  static char ID;
  InspectDependences() : ScopPass(ID) {
    /*Readings=0;
  		Writings=0;
  		ReadWrite=0;
  		WriteWrite=0;
...
void InspectDependences::checkLoopBodies(const clast_for *for_loop,
                                         int indent) {
...
void InspectDependences::printDependences(const char *description,
                                          Dependences::Type type) {
...
bool InspectDependences::runOnScop(Scop &scop) {
  ...
  assert(scop.getNumParams() == 0 &&
         "FIXME: "
         "After Constant Substitution number of scop's global parameters must "
         "be zero"
  ...
  if (((clast_stmt *)root)->next) {
    ...
    VERBOSE(Verbose::Polly << Verbose::Red
                           << "WARNING: There is useless Scop ( i.e. scop "
                              "without statements )!!!\n" << Verbose::Reset
                           << Verbose::Default);
    assert(scop.begin() == scop.end());
  }

  VERBOSE(Verbose::Polly << "<------------------------------ Scop: dependences "
                            "end ----------------------->\n"
                         << Verbose::Default);
...
Pass *createInspectDependencesPass() { return new InspectDependences(); }
...
static RegisterPass<InspectDependences> Z("InspectDependences",
                                          "Kernelgen - inspect dependences");
...
Pass *createScopDescriptionPass() { return new ScopDescription(); }

INITIALIZE_PASS_BEGIN(ScopDescription, "print-scop",
                      "Print cloog and polly's scop definitions", false, false)
INITIALIZE_PASS_DEPENDENCY(ScopInfo)
INITIALIZE_PASS_DEPENDENCY(CloogInfo)
...
public:
  ...
  SetRelationType(MemoryAccess::RelationType _relationType =
...
INITIALIZE_PASS_BEGIN(SetRelationType, "set-accesses-type",
                      "kernelgen's set current acceses type", false, false)
INITIALIZE_PASS_DEPENDENCY(ScopInfo)
INITIALIZE_PASS_END(SetRelationType, "set-accesses-type",
                    "kernelgen's set current relation type", false, false)
