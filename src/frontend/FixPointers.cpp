...
static RegisterPass<FixPointers> Z("fix-pointers",
                                   "Remove extra bit casts of malloc's calls");
Pass *createFixPointersPass() { return new FixPointers(); }
void FixPointers::FixPointersInModule(Module *m) {
  ...
  //for(vector<GetElementPtrInst *>::iterator GEPs_iterator = GEPs.begin(),
  //GEPs_iterator_end = GEPs.end();
  ...
  for (int i = 0; i < GEPs.size(); i++) {
    ...
    if (GEPInst->getNumIndices() == 1 &&
        ...
        case Instruction::Mul: { // index is realIndex*indexCoefficient
          ...
          if (!isa<Constant>(*instructionIndex->getOperand(0)) &&
            ...
            ConstantInt *secondOperand =
                cast<ConstantInt>(instructionIndex->getOperand(1));
      ...
      if (!isInstruction && !isConstant)
        continue;

      for (Value::use_iterator userOfGep = GEPInst->use_begin(),
                               userOfGep_end = GEPInst->use_end();
        ...
        if (isa<BitCastInst>(**userOfGep)) {
          ...
          if (castInst->getDestTy()->isPointerTy()) {
            PointerType *newPointerType =
                cast<PointerType>(castInst->getDestTy());
            int typeAllocSize =
                DL.getTypeAllocSize(newPointerType->getElementType());

            /*{
      							outs() << "<------------------------------- One more GEP Inst
      ------------------------------->" << "\n";
      							PointerType * asdf =
      cast<PointerType>(cast<Instruction>(GEPInst->getOperand(0))->getOperand(0)->getType());
      							outs() << " Real type : ";
      							...
      							if(isInstruction) {
      								assert(instIndex);
      								outs().indent(6) << *instIndex << " with coefficient " <<
      indexCoefficient << "\n";
      							}
      							if(isConstant) {
      ...
      << "\n\n";
            ...
            if (typeAllocSize == 0) {
              ...
              if (elementType->isArrayTy()) {
                ...
                outs() << "KernelGen can not remove bit cast: typeAlloc size "
                          "is zero\n";
                outs().resetColor();
                continue;
              }
            ...
            Instruction *newBitCast = new BitCastInst(
                GEPInst->getOperand(0), newPointerType, "newBitCast");
            //newBitCast ->
            //insertAfter(cast<Instruction>(GEPInst->getOperand(0)));
            ...
            if (isConstant && (constantIndex % typeAllocSize == 0)) {
              ...
              Value *newIndex = ConstantInt::getSigned(
                  GEPIndex->getType(), constantIndex / typeAllocSize);
              vector<Value *> Idx;
              Idx.push_back(newIndex);
              newGEPInst =
                  GetElementPtrInst::Create(newBitCast, Idx, "newGEPInst");
            } else if (isInstruction &&
                       (indexCoefficient % typeAllocSize == 0)) {

              Instruction *newIndex = instructionIndex->clone();
              newIndex->setOperand(
                  1, ConstantInt::getSigned(instructionIndex->getType(),
                                            indexCoefficient / typeAllocSize));
            ...
            if (newGEPInst) {
              ...
              if (newPointerType != castInst->getDestTy()) {
                // bit cast to array
                Instruction *newBitCastToArray = new BitCastInst(
                    newGEPInst, castInst->getDestTy(), "newBitCastToArray");
...
static RegisterPass<MoveUpCasts> A("move-up-casts",
                                   "Move each bitcast to it's argument");
Pass *createMoveUpCastsPass() { return new MoveUpCasts(); }
void MoveUpCasts::MoveUpCastsInModule(Module *m) {
  ...
  /*map<BitCastInst *,BitCastInst *> equalBitCasts;
 	...
 	for(map<BitCastInst *,BitCastInst *>::iterator equalPair =
 equalBitCasts.begin(),equalPair_end = equalBitCasts.end();
