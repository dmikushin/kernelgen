...
public:
  BranchedCodeExtractor(DominatorTree *dt = 0, bool _makeBranch = 0)
      : makeBranch(_makeBranch), DT(dt),
        NumExitBlocks(~0U) /*, OriginalLoopBlocks(BlocksToExtract)*/ {}
...
private:
  ...
  void makeFunctionBody(Function *LoopFunction, BasicBlock *header,
                        BasicBlock *clonedHeader, Values inputs, Values outputs,
  ...
  void createLoadsAndSwitch(CallInst *callLoopFuncInst, Values &inputs,
                            Values &outputs, BasicBlock *loadAndSwitchExitBlock,
...
void BranchedCodeExtractor::severSplitPHINodes(BasicBlock *&Header) {
  ...
  if (NumPredsFromRegion) {
    ...
    for (AfterPHIs = OldPred->begin(); isa<PHINode>(AfterPHIs); ++AfterPHIs) {
      ...
      // All uses of old phi-nodes in BlocksToExtract replace by uses of new
      // phi-nodes
      for (Value::use_iterator Usr = PN->use_begin(), Usr_end = PN->use_end();
           Usr != Usr_end; Usr++) {
    ...
    for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
      ...
      for (User::op_iterator O = I->op_begin(), E = I->op_end(); O != E; ++O)
        if (definedInCaller(*O) &&
            ((*O)->getType()->isIntegerTy() || (*O)->getType()->isPointerTy()))
...
bool BranchedCodeExtractor::isEligible(const std::vector<BasicBlock *> &code) {
  ...
  for (std::vector<BasicBlock *>::const_iterator BB = code.begin(),
                                                 e = code.end();
       BB != e; ++BB)
...
SetVector<BasicBlock *> *CloneCodeRegion(const SetVector<BasicBlock *> &code,
  ...
  for (SetVector<BasicBlock *>::const_iterator BI = code.begin(),
                                               BE = code.end();
       BI != BE; ++BI) {
...
void BranchedCodeExtractor::makeFunctionBody(
    Function *LoopFunction, BasicBlock *header, BasicBlock *clonedHeader,
    Values inputs, Values outputs, std::vector<BasicBlock *> &ExitBlocks,
    ValueToValueMapTy &allToAllMap) {
  ...
  Function::BasicBlockListType &functionBlocks =
      LoopFunction->getBasicBlockList();
  for (SetVector<BasicBlock *>::const_iterator i = ClonedLoopBlocks.begin(),
                                               e = ClonedLoopBlocks.end();
       i != e; ++i) {
  ...
  for (Values::const_iterator I = inputs.begin(), E = inputs.end(); I != E;
       ++I) {
    if ((*I)->getType() == i1Ty) {
  ...
  for (unsigned i = 0, e = inputs.size(); i != e; i++) {
    ...
    for (std::vector<User *>::iterator user = Users.begin(),
                                       userE = Users.end();
         user != userE; ++user)
  ...
  // over all of the blocks in the fuction body (ClonedLoopBlocks), updating any
  // terminator
      ...
      if (!ClonedLoopBlocks.count(TI->getSuccessor(i))) {
        ...
        if (!NewTarget) {
          ...
          NewTarget = BasicBlock::Create(
              context, OldTarget->getName() + ".exitStub", LoopFunction);
          ...
          for (unsigned out = 0, e = outputs.size(); out != e; ++out) {
            ...
            GetElementPtrInst *GEP = GetElementPtrInst::Create(
                structArg, Idx, "store_ptr_" + outputs[out]->getName(), NTRet);

            if (outputs[out]->getType() == i1Ty) {
              // Special case: cast i1 to i8, if output type is i1.
              CastInst *CI;
              if (makeBranch)
                CI = CastInst::CreateIntegerCast(allToAllMap[outputs[out]],
                                                 i8Ty, false, "", NTRet);
              else
                CI = CastInst::CreateIntegerCast(outputs[out], i8Ty, false, "",
                                                 NTRet);
  ...
  assert(NumExitBlocks == ExitBlocks.size() &&
         "have to handle all exit blocks");

  return;
}

CallInst *BranchedCodeExtractor::createCallAndBranch(
    Function *LaunchFunc, Function *KernelFunc, Values &inputs, Values &outputs,
  ...
  ArgTypes.push_back(Type::getInt8PtrTy(context));
  for (Values::iterator v = StructValues.begin(), ve = StructValues.end();
       v != ve; ++v) {
    if ((*v)->getType() == i1Ty) {
      // Special case: store i1 type as i8.
  ...
  StructType *StructArgTy =
      StructType::get(context, ArgTypes, false /* isPacked */);
  ...
  CallInst *MI = Builder.CreateMemSet(
      Struct, Constant::getNullValue(Type::getInt8Ty(context)),
  ...
  for (unsigned i = 0, e = inputs.size(); i != e; ++i) {
    ...
    if (StructValues[i]->getType() == i1Ty) {
      ...
      CastInst *CI = CastInst::CreateIntegerCast(StructValues[i], i8Ty, false,
                                                 "", callAndBranchBlock);
  ...
  Value *name[] = { ConstantDataArray::getString(context, KernelFunc->getName(),
                                                 true) };
  MDNode *nameMD = MDNode::get(context, name);

  // Add pointer to the original function string name
  // (to be set later on).
  params.push_back(Constant::getNullValue(Type::getInt8PtrTy(context)));
  ...
  Constant *size = Constant::getNullValue(Type::getInt64Ty(context));
  if (numints) {
    size = ConstantExpr::getSub(
        // The offset of the last integer argument.
        ConstantExpr::getAdd(
            ConstantExpr::getOffsetOf(StructArgTy, (numints - 1) + 2),
  ...
  CallInst *call = CallInst::Create(LaunchFunc, params,
  ...
  if (makeBranch) {
    Value *Cond = new ICmpInst(
        *callAndBranchBlock, ICmpInst::ICMP_EQ, call,
        ConstantInt::get(Type::getInt32Ty(context), KERNELGEN_STATE_FALLBACK));
    BranchInst::Create(header, loadAndSwitchExitBlock, Cond,
                       callAndBranchBlock);
  } else
  ...
  for (unsigned out = 0, e = outputs.size(); out != e; ++out) {
    ...
    if (outputs[out]->getType() == i1Ty) {
      // Special case: cast i1 to i8, if input type is i1.
      CastInst *CI = CastInst::CreateIntegerCast(GEP, i8Ty, false, "",
                                                 loadAndSwitchExitBlock);
      if (makeBranch)
...
void BranchedCodeExtractor::createLoadsAndSwitch(
    CallInst *callLoopFuncInst, Values &inputs, Values &outputs,
    BasicBlock *loadAndSwitchExitBlock,
    const std::vector<BasicBlock *> &ExitBlocks,
    ValueToValueMapTy &OutputsToLoadInstMap, AllocaInst *Struct) {
  ...
  SwitchInst *TheSwitch = SwitchInst::Create(
      callLoopFuncInst,
      loadAndSwitchExitBlock, // заменить на блок, который
                              // выдавал бы ошибку
      NumExitBlocks ? NumExitBlocks : 1,
      loadAndSwitchExitBlock);
...
void BranchedCodeExtractor::updatePhiNodes(
    ...
    BasicBlock *loadAndSwitchBlock, std::vector<BasicBlock *> &ExitBlocks) {
  if (NumExitBlocks == 0)
    return;
  if (outputs.size() == 0)
    return;
  ...
  unsigned NumPreds = 0;
  for (pred_iterator predBlock = pred_begin(ExitBlock),
                     predBlockE = pred_end(ExitBlock);
  ...
  for (Values::iterator Out = outputs.begin(), Outputs_end = outputs.end();
    ...
    for (BasicBlock::iterator Inst = ExitBlock->begin(),
                              E = ExitBlock->getFirstNonPHI();
         Inst != E; Inst++) {
    ...
    PHINode *newPN = PHINode::Create(I->getType(), NumPreds, "newPHINode",
                                     ExitBlock->begin());
    ...
    for (pred_iterator predBlock = pred_begin(ExitBlock),
      ...
      if (*predBlock != loadAndSwitchBlock) {
        if (OriginalLoopBlocks.count(*predBlock))
          newPN->addIncoming(I, *predBlock);
        else
          newPN->addIncoming(UndefValue::get(I->getType()), *predBlock);
      } else
        newPN->addIncoming(OutputsToLoadInstMap[I], loadAndSwitchBlock);

    for (SetVector<Value *>::iterator UI = Users.begin(), E = Users.end();
         UI != E; ++UI) {
...
///
CallInst *BranchedCodeExtractor::ExtractCodeRegion(Loop *L, LoopInfo &LI) {
  ...
  Values inputs;
  Values
      outputs; // instructions in !!OriginalLoop!!, which used outside the Loop
  ...
  for (unsigned i = 1, e = code.size(); i != e; ++i)
    for (pred_iterator PI = pred_begin(code[i]), E = pred_end(code[i]); PI != E;
         ++PI) {
  ...
  if (NumExitBlocks != 1) {
    ...
    outs() << "KernelGen dropped loop: There must be only one exit block from "
              "the code region\n";
    outs().resetColor();
    return NULL;
  }
  ...
  if (makeBranch) {
    clonedCode.reset(CloneCodeRegion(BlocksToExtract, RF_IgnoreMissingEntries,
                                     VMap, ".cloned", &CodeInfo));
  ...
  BasicBlock *loadAndSwitchExitBlock = BasicBlock::Create(
      header->getContext(), "loadOutputsAndSwitchExit", parentFunction, header);
  BasicBlock *callAndBranchBlock =
      BasicBlock::Create(header->getContext(), "callAndBranch", parentFunction,
                         loadAndSwitchExitBlock);
  ...
  if ((parentLoop = (L->getParentLoop()))) {
    ...
    if (!makeBranch) {
      ...
      for (SetVector<BasicBlock *>::iterator iter = BlocksToExtract.begin(),
                                             iter_end = BlocksToExtract.end();
           iter != iter_end; iter++) {
  ...
  if (!launchFunction)
    launchFunction =
        Function::Create(TypeBuilder<types::i<32>(types::i<8> *, types::i<64>,
                                                  types::i<64>, types::i<32> *),
                                     true>::get(context),
                         GlobalValue::ExternalLinkage, "kernelgen_launch", m);

  // Construct new loop function based on inputs/outputs & add allocas for all
  // defs.
  ...
  // Make function body.
  std::vector<BasicBlock *> ExitBlocks;
  makeFunctionBody(loopFunction, header, clonedHeader, inputs, outputs,
                   ExitBlocks, VMap);

  // Make call and make branch after call.
  AllocaInst *Struct;
  CallInst *callLoopFuctionInst = createCallAndBranch(
      launchFunction, loopFunction, inputs, outputs, callAndBranchBlock, header,
      loadAndSwitchExitBlock, Struct, VMap);
  ...
  for (unsigned i = 0, e = Users.size(); i != e; i++) {
    ...
    if (TerminatorInst *TI = dyn_cast<TerminatorInst>(Users[i]))
      if (!OriginalLoopBlocks.count(TI->getParent()) &&
        ...
        for (BasicBlock::iterator PN = header->begin(); isa<PHINode>(PN);
             PN++) {
  ...
  if (makeBranch) {
    ...
    updatePhiNodes(outputs, OutputsToLoadInstMap, loadAndSwitchExitBlock,
                   ExitBlocks);
  } else {
    //BlocksToExtract.insert(ExitBlocks.begin(),ExitBlocks.end());
    SetVector<BasicBlock *> blocksOfLoopFunction;
    for (Function::iterator iter = loopFunction->begin(),
                            iter_end = loopFunction->end();
         iter != iter_end; iter++)
    ...
    for (Values::iterator Out = outputs.begin(), Outputs_end = outputs.end();
      ...
      for (Value::use_iterator iter = (*Out)->use_begin(),
                               iter_end = (*Out)->use_end();
           iter != iter_end; iter++)
      ...
      for (list<Instruction *>::iterator iter = needToReplaceUses.begin(),
                                         iter_end = needToReplaceUses.end();
           iter != iter_end; iter++)
