...
static match match_deferred_coarray_spec(g95_coarray_spec *cas) {
	...
	if (++cas->rank >= G95_MAX_DIMENSIONS) {
...
int i;
    ...
    for(;;) {
	if (g95_match_char('*') == MATCH_YES) {
	    cas->lower[cas->rank++] = g95_int_expr(1);
	    break;
	}

	m = g95_match_expr(&cas->lower[cas->rank]);
	...
	if (g95_match_char(':') != MATCH_YES) {
	    cas->upper[cas->rank] = cas->lower[cas->rank];
	    cas->lower[cas->rank] = g95_int_expr(1);
	    ...
	    m = g95_match_expr(&cas->upper[cas->rank]);
	    if (m == MATCH_ERROR)
		return m;
    ...
    if (cas->rank == 0 || cas->upper[cas->rank - 1] != NULL) {
...
error:
    ...
    if (ar_p != NULL) *ar_p = &ref->u.ar;
    if (as_p != NULL) *as_p = as;
}
