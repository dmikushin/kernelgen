...
FILE *g95_open_included_file(char *name,char *pfullname) {
...
int m;
    ...
    /* Getting desparate now, try the system directories */
...
int size, n;
	...
	for(c=ts->derived->components; c; c=c->next) {
		...
		n += size * bi_to_int(g95_array_spec_size(c->as));
	}
...
int errors, warnings, i;
    ...
    if( g95x_option.intrinsics ) {
        g95x_start( NULL );
        g95x_print_intrinsics();
        g95x_end();
        goto done;
    }
