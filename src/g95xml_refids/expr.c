...
g95x_kind *g95x_get_kind( g95_namespace *ns ) {
g95x_kind *xk;

    xk = g95_getmem( sizeof( g95x_kind ) );
...
bignum *p;
    ...
    if (g95_current_shapes == NULL || next_shape + n >= G95_SHAPES) {
	s = g95_getmem(sizeof(g95_shapes));
	s->next = g95_current_shapes;
	g95_current_shapes = s;
	next_shape = 0;
    }

    p = g95_current_shapes->buffer + next_shape;
    next_shape += n;
...
#ifdef G95X_CHECK
        if( count > 0 ) 
            g95x_die( "Nested x.full exprs\n" );
        count++;
#endif
        g95_free_expr( e->x.full );
#ifdef G95X_CHECK
        count--;
#endif
    }
...
g95_expr *full = dest->x.full;
g95x_locus xw = dest->x.where;
...
char *s;
    ...
    *q = *p;
    ...
    if( q->x.full ) 
        q->x.full = g95_copy_expr( q->x.full );
    ...
    if( q->x.boz ) {
        q->x.boz = g95_getmem( strlen( p->x.boz ) + 1 );
        strcpy( q->x.boz, p->x.boz );
    }
...
/* g95_has_coarray()-- Return nonzero if the expression has a coarray
 * reference. */

int g95_has_coarray(g95_expr *e) {
g95_ref *ref;
...
g95_symbol *g95_get_temporary(g95_typespec *ts, int rank) {
...
g95_expr *f;
	...
	if (e->value.function.actual->next == NULL) {
	    a = g95_get_actual_arglist();
	    a->type = ARG_EXPR;
	    e->value.function.actual->next = a;

	    break;
	}

	if (e->value.function.actual->next->next != NULL)
	    return 0;
...
int i;
    ...
    for(s=g95_current_shapes; s; s=t) {
	t = s->next;
