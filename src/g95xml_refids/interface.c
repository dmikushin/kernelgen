 ...
 * the SEQUENCE attribute to be equal */
	...
	ts1->derived->component_access == ACCESS_PRIVATE ||
...
g95x_locus xw;
	...
	*operator = INTRINSIC_USER;
    ...
    if (g95_match_name(buffer) == MATCH_YES) {
        g95x_ext_locus* exw = g95x_get_current_ext_locus_ref();
        exw->type = G95X_LOCUS_GNRC;
	strcpy(name, buffer);
 ...
 * satisifes rule 2, nonzero otherwise.  This test is not symmetric in
...
int i;
	...
	formal->result != NULL && actual->symbol->result != NULL &&
	!compare_type_rank(formal->result, actual->symbol->result)) {
	if (error_flag)
...
comparison c;
	...
	if (actual->type != EXPR_VARIABLE) {
		...
		g95_error("Argument for coarray parameter '%s' at %L must "
			  "be a VARIABLE", formal->name, &actual->where);
...
static try check_intent(g95_intent f_intent, g95_expr *a, int pointer) {
g95_intent a_intent;
g95_symbol *sym;

    if (a->type == EXPR_CONSTANT) {
	if (f_intent == INTENT_OUT || f_intent == INTENT_INOUT) {
	    g95_error("Constant actual argument %L to INTENT(%s) dummy "
		      "argument", &a->where, g95_intent_string(f_intent));
	    return FAILURE;
	}
	...
	if (f_intent == INTENT_OUT || f_intent == INTENT_INOUT) {
	    g95_error("Non-variable argument %L to INTENT(%s) dummy argument", 
		      &a->where, g95_intent_string(f_intent));
	    return FAILURE;
	}
	...
	(f_intent == INTENT_OUT || f_intent == INTENT_INOUT)) {
	g95_error("PARAMETER '%s' is associated with an INTENT(%s) dummy "
		  "argument at %L", sym->name, g95_intent_string(f_intent),
		  &a->where);
	return FAILURE;
    }
    ...
    switch(f_intent) {
    case INTENT_UNKNOWN:
	g95_set_usage(sym, &a->where, 1, 1);  /* Conservative assumption */
	break;

    case INTENT_INOUT:
	g95_set_usage(sym, &a->where, 1, 1);
	break;

    case INTENT_IN:
	g95_set_usage(sym, &a->where, 0, 1);
	break;

    case INTENT_OUT:
	g95_set_usage(sym, &a->where, 1, 0);
	break;
    }
...
try t;
	...
	if (st_function)
	    f_intent = INTENT_UNKNOWN;

	if (check_intent(f_intent, a->u.expr, a->pointer) == FAILURE)
	    t = FAILURE;
    }
...
g95_actual_arglist *a;
	...
	if (compare_actual_formal(ap, intr->sym->formal,
	    ...
	    check_intents(intr->sym->formal, *ap, 0);
	    return intr->sym;
	}
    }
 ...
 * useless.  Returns 0 if everything is OK, 1 if there an error or 2
 * if there wasn't a self-interface issue. */
...
g95_interface *intf = NULL;
	...
	sym = g95_search_interface(ns->operator[INTRINSIC_ASSIGN], 1, &actual);
        if (sym != NULL) {
            intf = ns->operator[INTRINSIC_ASSIGN];
            break;
        }
    }
...
g95_symtree *st;
	...
	for(ns=g95_current_ns; ns; ns=ns->parent) {
        ...
        if( sym != NULL ) 
            st = g95_find_symtree( ns->uop_root, e->value.op.uop->name );
...
/* type_warning()-- Warn abour inconsistent types. */
...
//GEN  int length = strlen( current_interface.generic->name );
//GEN  intr->x.name = g95_getmem( length + 1 );
//GEN  strcpy( intr->x.name, current_interface.generic->name );
