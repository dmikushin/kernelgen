...
try t;
    ...
    if (f->sym->attr.intent == INTENT_UNKNOWN &&
	f->sym->ts.type != BT_PROCEDURE && !f->sym->attr.function &&
	!f->sym->attr.subroutine && !f->sym->attr.pointer &&
	proc->attr.proc != PROC_ST_FUNCTION) {

	if (G95_STRICT_F()) {
	    g95_error("Actual argument '%s' at %L does not have an INTENT",
		      f->sym->name, &f->where);
	    return FAILURE;
	}
...
g95_formal_arglist *f;
	...
	if (resolve_formal_arg(proc, f) == FAILURE)
	    return FAILURE;
...
int rank;
    ...
    if( g95_resolve_expr( e->value.function.pointer ) == FAILURE )
        return FAILURE;
...
g95_symbol *sym;
    ...
    if( g95_resolve_expr( c->ext.sub.pointer ) == FAILURE )
        return FAILURE;
 ...
 * type components must also have SEQUENCE. */
	...
	/* Constaint in f95 4.4.1, apparently removed in F2003. */
...
/* derived_private()-- Given a derived type, return nonzero if it has
 * any private components or subtypes with private components. */

static int derived_private(g95_symbol *sym) {
g95_component *c;
...
/* pointer_component()-- Given a pointer to a symbol that is a derived
...
static int pointer_component(g95_symbol *sym) {
g95_component *c;
	...
	if (c->ts.type == BT_DERIVED &&
	    pointer_component(c->ts.derived))
	    return 1;
    }
...
/* coarray_component()-- Return nonzero if a derived type has a
 * coarray at some level. */

static int coarray_component(g95_symbol *sym) {
g95_component *c;
	...
	if (!c->pointer && c->ts.type == BT_DERIVED &&
	    coarray_component(c->ts.derived))
	    return 1;
    }
...
int i;
	...
	if (g95_symbol_access(sym) == ACCESS_PUBLIC) {
	    ...
	    if (s->ts.type == BT_DERIVED && derived_private(s->ts.derived)) {
	...
	if (pointer_component(s->ts.derived)) {
...
try t;
	...
	g95_error("Module variable '%s' at %L with a default initialization "
    ...
    if (sym->attr.modproc && !sym->attr.contained && !sym->attr.entry) {
	g95_error("MODULE PROCEDURE '%s' at %L never appeared in a module",
		  sym->name, &sym->declared_at);
	return FAILURE;
    }
	...
	if (!sym->attr.dummy && !g95_module_symbol(sym) && !sym->attr.save) {
...
/* Typecheck the new node. */
	...
	if ((e->value.op.operator == INTRINSIC_EQ ||
	     e->value.op.operator == INTRINSIC_NE) &&
...
g95_typespec ts;
    ...
    if (!data_flag && index->ts.kind != g95_default_integer_kind(0)) {
	g95_clear_ts(&ts);
	ts.type = BT_INTEGER;
...
int i;
    ...
    if (spec->rank != ref->dimen) {
	g95_error("Corank reference mismatch (%d/%d) at %L",
		  spec->rank, ref->dimen, where);
	return FAILURE;
    }
...
g95_ref *ref;
	...
	case REF_COARRAY:
	    if (cas == NULL) {
		g95_error("Unexpected coarray reference in expression at %L", &ref->where);
		return FAILURE;
	    }
...
g95_symtree *st;
	...
	coarray_component(sym->result->ts.derived)) {
...
try t;
    ...
    if( e->x.full ) {
        g95x_simplify_push( 1 );
        g95_resolve_expr( e->x.full );
        g95x_simplify_pop();
    }
...
int i, m, pointer, allocatable, dimension, seen_section;
g95_ref *ref, *ref2;
g95_array_ref *ar;
g95_symbol *sym;
try t;
	...
	if (e->type == EXPR_FUNCTION)
	    dimension = e->symbol->result->attr.dimension;
	...
	if (is_protected(sym)) {
	    g95_error("Cannot ALLOCATE a PROTECTED pointer '%s' at %L",
		      sym->name, &e->where);
	    return FAILURE;
	}
	...
	for(ref=e->ref; ref; ref2=ref, ref=ref->next)
	    ...
	    case REF_COARRAY:
		g95_error("Cannot ALLOCATE on another image at %L", &ref->where);
		return FAILURE;
    ...
    if (e->type == EXPR_VARIABLE && e->symbol->attr.dummy &&
	e->symbol->attr.allocatable && e->symbol->attr.intent == INTENT_IN) {
    ...
    if (ref2 == NULL || ref2->type != REF_ARRAY ||
	ref2->u.ar.type == AR_FULL) {
...
int m;
    ...
    if (lhs->ts.type == BT_DERIVED && pointer_component(lhs->ts.derived) &&
...
try rc, t;
	...
	case EXEC_ALLOCATE:
		...
		if (resolve_allocate_expr(a->expr) == FAILURE) {
		    rc = FAILURE;
		    break;
		}
	...
	case EXEC_SYNC_IMAGES:
		...
		if (code->expr->rank != 0) {
		    g95_error("Image number at %L must be a scalar",
			      &code->expr->where);
		    rc = FAILURE;
		}
	    }
...
static void check_usage(g95_symbol *sym) {
	...
	if (g95_option.unused_module_vars && !sym->attr.used && !sym->attr.set
	    && !sym->attr.alloced && !sym->attr.dealloced)
	...
	g95_warning(112, "Variable '%s' at %L is set but never used",
	       ...
	       !sym->attr.allocatable && sym->attr.intent != INTENT_OUT) {
...
g95_ref *ref;
	...
	if (pointer_component(sym->ts.derived)) {
