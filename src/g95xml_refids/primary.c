...
match m;
...

    {
        ...
        g95x_push_locus( &xw, G95X_LOCUS_KWRD ); /* so that keywords are not recognized
                                                    as names */
    }
...
match m;
	...
	*result = base;
    ...
    if (base->ts.interface->attr.function) {
	...
	*result = e;
        ...
        g95x_set_kwrd_locus_expr( e );

    } else if (base->ts.interface->attr.subroutine)
	*result = base;

    else
	g95_internal_error("pointer_result(): Bad case");
...
match g95_match_call_expr(g95_symbol *sym, g95_expr **result) {
    ...
    return match_variable_part(sym, result, 0);
}
