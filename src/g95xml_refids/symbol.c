...
g95_ff *ff;
    ...
    result->x.where = e->x.where;
    result->x.boz = e->x.boz;
    e->x.boz = NULL;
...
int is_pure;
    ...
    if( ( rvalue->type == EXPR_FUNCTION )
     && ( rvalue->value.function.isym )
     && ( ! strcmp( rvalue->value.function.name, "null" ) )
    ) return SUCCESS; 
...
int i;
	...
	for(c=e->value.constructor.c; c; c=c->next) {
	    ...
	    stack.new = g95_get_temporary(&ts, 0);
