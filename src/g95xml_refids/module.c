...
static mstring iprocs[] = {
    ...
    minit("NUM_IMAGES",   IPROC_NUM_IMAGES),
    minit("CO_LBOUND",    IPROC_CO_LBOUND),
    minit("CO_UBOUND",    IPROC_CO_UBOUND),
    minit("IMAGE_INDEX",  IPROC_IMAGE_INDEX),
    minit("THIS_IMAGE",   IPROC_THIS_IMAGE),
    minit(NULL, -1)
};
...
match m;
	...
	if (mod_kind != MOD_UNKNOWN && g95_match(" ::") == MATCH_NO)
	    goto syntax;
    }
	...
	case INTERFACE_GENERIC:
            ...
            new->use_name_el = g95x_get_current_ext_locus_ref();
    
	    m = g95_match(" =>");
      
            if( m == MATCH_YES ) {
                new->local_name_el = new->use_name_el;
                new->use_name_el = NULL;
            }
	...
	case INTERFACE_USER_OP:
            ...
            new->use_name_el = g95x_get_current_ext_locus_ref() - 2;
...
/* Faire une liste de symtree */
	...
	for(;;) {
	    ...
	    set_module_locus(&m);
            ...
            set_local_name_locus_uop( p, name, intf );

            for( ; intf; intf = intf->next ) { 
                intf->x.module = g95_get_string( module_name );
                intf->x.name = g95_getmem( strlen( name ) + 1 );
                strcpy( intf->x.name, name );
            }         
	}
...
/* Faire une liste de symtree */
	    ...
	    set_module_locus(&m);
            ...
            set_local_name_locus_gen( p, atom_string, intf );

            for( ; intf; intf = intf->next ) { 
                intf->x.module = g95_get_string( module_name );
                intf->x.name = g95_getmem( strlen( atom_string ) + 1 );
                strcpy( intf->x.name, atom_string );
            }
...
pointer_info *p;
    ...
    if (!check_access(sym->attr.access, sym->ns->default_access) ||
	sym->attr.st_construct || check_unique_name(st->name))
	return;
...
/* g95_module_parameter()-- Create a parameter for an intrinsic module. */

void g95_module_parameter(char *name, g95_expr *value) {
char *local_name;
g95_symbol *sym;
g95_symtree *st;
