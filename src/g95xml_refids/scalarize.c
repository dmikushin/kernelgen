...
static void section_from_section(g95_array_ref *, g95_array_ref *,
 ...
 * The only way an expresion can be nonelemental is to have a
...
g95_symbol *var;
    ...
    default:
	var = g95_get_temporary(&e->ts, e->rank);
...
static g95_expr *full_array(g95_expr *e) {
g95_ref *ref, *r;
g95_typespec *ts;
int i;
...
static void section_from_bounds(g95_array_ref *ar, g95_expr *e,
				int copy_bounds) {
int i, rank;

    rank = ar->dimen = e->rank;

    for(i=0; i<rank; i++) {
	ar->dimen_type[i] = DIMEN_RANGE;

	if (copy_bounds) {
	    ar->start[i] = bound_expr(e, i, 0);
	    ar->end[i] = bound_expr(e, i, 1);

	} else {
	    ar->start[i] = g95_int_expr(1);
	    ar->end[i] = get_extent(bound_expr(e, i, 0),
...
static void section_from_spec(g95_array_ref *ar, g95_array_spec *as) {
int i, rank; 

    rank = ar->dimen = as->rank;

    for(i=0; i<rank; i++) {
	ar->dimen_type[i] = DIMEN_RANGE;

	ar->start[i] = g95_int_expr(1);
	ar->end[i]   = get_extent(g95_copy_expr(as->lower[i]),
...
static void section_range(g95_array_ref *ar, g95_array_ref *src,
			  g95_array_spec *as, g95_expr *array,
			  int i, int rank) {
g95_expr *start, *end, *step;

    ar->dimen_type[rank] = DIMEN_RANGE;
    ar->start[rank] = g95_int_expr(1);
    ...
    ar->end[rank] = get_extent(start, end, step);
}
...
static int section_vector(g95_expr *e, g95_array_ref *ar, int i) {
g95_actual_arglist *a;
g95_array_spec *as;
g95_array_ref ar0;
g95_ref *ref;
    ...
    switch(e->type) {
    case EXPR_OP:
	if (section_vector(e->value.op.op1, ar, i) ||
	    section_vector(e->value.op.op2, ar, i))
	    return 1;
	...
	for(a=e->value.function.actual; a; a=a->next)
	    if (section_vector(a->u.expr, ar, i))
		return 1;
	...
	for(ref=e->ref; ref; ref=ref->next)
		...
		case AR_FULL:
		    section_range(ar, &ref->u.ar, as, e, 0, i);
		    return 1;

		case AR_SECTION:
		    memset(&ar0, '\0', sizeof(ar0));

		    section_from_section(&ar0, &ref->u.ar, as, e);

		    ar->start[i]      = ar0.start[0];
		    ar->end[i]        = ar0.end[0];
		    ar->stride[i]     = ar0.stride[0];
		    ar->dimen_type[i] = ar0.dimen_type[0];
		    return 1;
...
static void section_from_section(g95_array_ref *ar, g95_array_ref *src,
				 g95_array_spec *as, g95_expr *array) {
int i, rank;
	...
	case DIMEN_RANGE:
	    section_range(ar, src, as, array, i, rank++);
	    break;

	case DIMEN_VECTOR:
	    section_vector(src->start[i], ar, rank++);
	    break;
...
static int extract_section(g95_expr *e, g95_array_ref *ar, int copy_bounds) {
g95_array_spec *as;
g95_ref *ref;
	...
	case REF_ARRAY:
	    switch(ref->u.ar.type) {
	    case AR_FULL:
		section_from_bounds(ar, e, copy_bounds);
		return 1;

	    case AR_SECTION:
		section_from_section(ar, &ref->u.ar, as, e);
		return 1;
...
static int find_section_spec(g95_expr *e, g95_array_ref *ar, int copy_bounds) {
g95_actual_arglist *arg;
int rc;
    ...
    switch(e->type) { 
    case EXPR_OP:
	rc = find_section_spec(e->value.op.op1, ar, copy_bounds) ||
	     find_section_spec(e->value.op.op2, ar, copy_bounds);
	break;

    case EXPR_VARIABLE:
	rc = extract_section(e, ar, copy_bounds);
	break;

    case EXPR_FUNCTION:
	if (!g95_elemental_function(e)) {
	    section_from_spec(ar, e->symbol->as);
	    rc = 1;
	} else {

	    rc = 0;
	    for(arg=e->value.function.actual; arg; arg=arg->next)
		if (find_section_spec(arg->u.expr, ar, copy_bounds)) {
		    rc = 1;
		    break;
		}
 ...
 * character function call.  The main difficuly is that we don't know
...
g95_expr *e;
    ...
    var = g95_get_temporary(&ts, rhs->rank);
    ...
    if (!find_section_spec(rhs, &alloc_ref->u.ar, 0))
...
int m;
	...
	r->var[j]   = var = g95_get_temporary_int();
...
int flag;
	...
	sym = g95_get_temporary(ts, variable_rank(e));
	sym->attr.pointer = 1;
...
g95_ref *ref;
     ...
     * could be avoided, but they are somewhat esoteric. */
	...
	var = g95_get_temporary(&n->expr2->ts, 0);
...
int rank, i;
	...
	f->value.function.isym = g95_find_function("min");
	f->value.function.actual = a = g95_get_actual_arglist();

	a->type = ARG_EXPR;
	a->u.expr = g95_get_variable_expr(result, &e->where);
	...
	f->value.function.isym = g95_find_function("max");
	f->value.function.actual = a = g95_get_actual_arglist();
...
loop_info *head;
    ...
    result = g95_get_temporary(&e->ts, 0);
...
int m;
    ...
    var = g95_get_temporary(&ts, e->rank);
 ...
 * array substript into an appropriate loop.  We just search for the
...
/* scalarize_ac0()-- Traverse an array constructor.  We
 * replace non-constructor array expressions with an equivalent array
 * variable. */
...
g95_expr *e;
    ...
    e->ts.kind = g95_default_integer_kind(0);    /* Must work with -i8 */
...
g95_expr *e;
    ...
    iter->var = g95_get_variable_expr(g95_get_temporary_int(), 0);
...
int rank;
    ...
    if (!find_section_spec(c->expr, &ref->u.ar, 0))
...
g95_expr *e;
    ...
    c = g95_get_code(EXEC_DEALLOCATE, NULL);
    c->expr = g95_get_variable_expr(g95_get_temporary_int(), where);
    c->ext.alloc_list = g95_get_alloc();
    ...
    if (!find_section_spec(rhs, &ref->u.ar, 1))
...
g95_code *n;
    ...
    var = g95_get_temporary(&c->expr->ts, 0);
    var->attr.pointer = 1;
...
found:
	...
	var = g95_get_temporary(&rhs->ts, 0);
