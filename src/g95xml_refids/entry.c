...
/* insert_symbol()-- Insert symbols into the master namespace from a
 * module namepsace. */

static g95_symbol *insert_symbol(g95_namespace *master, g95_symbol *sym) {
g95_namespace *module;
g95_symtree *st;
g95_symbol *s;
    ...
    st = g95_find_symtree(module->sym_root, sym->name);
    st->n.sym->refs--;
    st->n.sym->ns = master;
    st->n.sym->module = NULL;

    st->n.sym = s = g95_getmem(sizeof(g95_symbol));
	...
	s->result = insert_symbol(master, sym->result);
    
    else if (sym->result == NULL && sym->attr.function)
	s->result = s;
...
int i;
    ...
    ref->where = sym->declared_at;
    ref->type = REF_ARRAY;
    ref->u.ar.type = AR_SECTION;
    ref->u.ar.dimen = e->rank;

    for(i=0; i<e->rank; i++) {
	ref->u.ar.start[i] = g95_copy_expr(sym->as->lower[i]);
	ref->u.ar.end[i] = g95_copy_expr(sym->as->upper[i]);
	ref->u.ar.dimen_type[i] = DIMEN_RANGE;
    }
...
int rank, n;
    ...
    result = g95_get_temporary(ts, 0);  /* Result variable */
    result->attr.dummy = 1;
