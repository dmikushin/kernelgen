...
try result;
    ...
    if( g95x_simplify_none() ) {
        g95_expr *k1 = g95_copy_expr( k );

        g95x_simplify_push( 0 );
        g95_simplify_expr( k1 );
        g95x_simplify_pop();
...
g95_array_ref *ar;
    ...
    g95_find_array_ref(e, &ar, &as);

    if (as->type != AS_ASSUMED_SIZE || ar->type != AR_FULL)
	return SUCCESS;
...
int rank;
    ...
    g95_find_array_ref(array, NULL, &as);
...
g95_array_spec *as;
    ...
    if (source->rank == 0 || source->type != EXPR_VARIABLE)
	return SUCCESS;

    g95_find_array_ref(source, NULL, &as);
...
try g95_check_c_associated(g95_expr *c_ptr1, g95_expr *c_ptr2) {
    ...
    if (c_ptr1->ts.type != BT_DERIVED &&
	c_ptr1->ts.derived->attr.itype != ITYPE_C_PTR &&
	c_ptr1->ts.derived->attr.itype != ITYPE_C_FUNPTR) {
	must_be(c_ptr1, 0, "of type C_PTR or C_FUNPTR");
	return FAILURE;      
    }

    if (c_ptr2 != NULL && c_ptr2->ts.type != BT_DERIVED &&
	c_ptr2->ts.derived->attr.itype != ITYPE_C_PTR &&
	c_ptr2->ts.derived->attr.itype != ITYPE_C_FUNPTR) {
...
bignum size;
	...
	cptr->ts.derived->attr.itype != ITYPE_C_PTR) {
	must_be(cptr, 0, "of type C_PTR");
	return FAILURE;
    }
...
try g95_check_c_f_procpointer(g95_expr *cptr, g95_expr *fptr) {
    ...
    if (cptr->ts.type != BT_DERIVED ||
	cptr->ts.derived->attr.itype != ITYPE_C_FUNPTR) {
	must_be(cptr, 0, "of type C_FUNPTR");
	return FAILURE;
    }
    ...
    if (fptr->ts.type != BT_PROCEDURE || !fptr->symbol->attr.pointer) {
