...
#define match(string, subr, statement) \
  if (g95_match(string) == MATCH_YES) { mm = subr(); sta = statement; goto got_match; }
    ...
    match("sync image",   g95_match_sync_images, ST_SYNC_IMAGES)
...
char c;
	...
	if (g95_add_type(sym, &ts, NULL) == FAILURE)
        ...
        g95x_push_enumerator_list_symbol( sym );
...
match m;
	...
	m = g95_match_variable(&tail->expr, 0, 0, 0);
	if (m == MATCH_ERROR) goto cleanup;
	if (m == MATCH_NO) goto syntax;
...
g95x_locus xw;
	...
	(sym->attr.flavor == FL_VARIABLE && !sym->attr.pointer)) {
...
match m;
	...
	if (name[0] == '\0') {
		...
		*g = *h;
        ...
        g95x_push_common_list_common( h );
...
/* Grab the list of symbols for this common. */
	...
	tail = sym;
        ...
        g95x_push_common_list_symbol( sym );
...
static match match_sync_stat(int image_flag) {
match m;
...
/* g95_match_sync_team()-- Match the SYNC TEAM statment. */
